	...
	public function checkPayStatus(){
        ...
        if(empty($data)){
        	...
        	if($data['isPay']==1){
				...
				//获取用户手机号
				$user = model('home/users')->getFieldsById((int)session('WST_USER.userId'),'userPhone');
				if(empty($user['userPhone'])) $bindPhone = 0;
				else $bindPhone = 1;
		...
		if(empty($data)){
			...
			if($data['isPay']==1){
				...
				if($auction->orderId>0){
					...
					$this->assign('userAddress',$userAddress);
					//获取用户手机号
					$user = model('wechat/users')->getFieldsById((int)session('WST_USER.userId'),'userPhone');
					if(empty($user['userPhone'])) $bindPhone = 0;
					else $bindPhone = 1;
					$this->assign('bindPhone',$bindPhone);
					//获取省份
		...
		if(empty($data)){
			...
			if($data['isPay']==1){
				...
				if($auction->orderId>0){
					...
					$this->assign('userAddress',$userAddress);
					//获取用户手机号
					$user = model('mobile/users')->getFieldsById((int)session('WST_USER.userId'),'userPhone');
					if(empty($user['userPhone'])) $bindPhone = 0;
					else $bindPhone = 1;
					$this->assign('bindPhone',$bindPhone);
					//获取省份
