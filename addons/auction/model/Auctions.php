...
class Auctions extends Base{
    ...
    public function installMenu(){
    	Db::startTrans();
		try{
			$hooks = ['beforeCancelOrder','wechatDocumentUserIndexTools','mobileDocumentUserIndexTools','adminDocumentHookSummary'];
	...
	public function addMobileBtn(){
		...
		if(WSTDatas('ADS_TYPE',4)){
			...
			$data["btnImg"] = "addons/auction/view/app/img/auction.png";
			$data["addonsName"] = "Auction";
			$data["btnSort"] = 6;
			Db::name('mobile_btns')->insert($data);
		}
		...
		if(WSTDatas('ADS_TYPE',5)){
			...
			$data["btnImg"] = "addons/auction/view/weapp/img/auction.png";
			$data["addonsName"] = "Auction";
			$data["btnSort"] = 6;
			Db::name('mobile_btns')->insert($data);
		}
	...
	public function pageQueryByShop($sId=0){
		...
		$where[]=['au.dataFlag','=',1];
		$where[]=['g.dataFlag','=',1];
		if($goodsName !='')$where[] = ['au.goodsName|g.goodsSn', 'like', '%'.$goodsName.'%'];
    ...
    public function searchGoods($sId=0){
    	...
    	$rs = Db::name('goods')->where($where)->field('goodsName,goodsId,marketPrice,shopPrice,goodsSeoKeywords,goodsSeoDesc')->select();
	...
	public function getById($id,$sId=0){
		...
		$where = [];
		$where['gu.shopId']=($sId==0)?(int)session('WST_USER.shopId'):$sId;
		$where['gu.auctionId'] = $id;
		$where['gu.dataFlag'] = 1;
		$where['gu.dataFlag'] = 1;
		$rs =  $this->alias('gu')->join('__GOODS__ g','g.goodsId=gu.goodsId','left')->where($where)->field('gu.goodsName,gu.goodsImg,g.marketPrice,g.shopPrice,gu.*')->find();
        ...
        $rs['auctionDesc'] = str_replace('${DOMAIN}',$resourceDomain,$rs['auctionDesc']);
        return $rs;
	}
	...
	public function add($sid=0){
		...
		$resourceDomain = ($isApp==1)?WSTConf('CONF.resourceDomain'):WSTConf('CONF.resourcePath');
		$data['auctionDesc'] = htmlspecialchars_decode($data['auctionDesc']);
        $auctionDesc = str_replace($resourceDomain.'/upload/','${DOMAIN}/upload/',$data['auctionDesc']);
     	$data["auctionDesc"] = WSTRichEditorFilter($auctionDesc);

		$data['shopId'] = $shopId;
		$data['goodsName'] = $goodsName;
		$data['goodsSeoKeywords'] = input('goodsSeoKeywords');
		$data['goodsSeoDesc'] = input('goodsSeoDesc');
	...
	public function edit($sid=0){
		...
		$auction->goodsName = $goodsName;
		$auction->goodsSeoKeywords = input('goodsSeoKeywords');
		$auction->goodsSeoDesc = input('goodsSeoDesc');
		$auction->auctionStatus = 0;
		$auction->updateTime = date('Y-m-d H:i:s');
		$resourceDomain = ($isApp==1)?WSTConf('CONF.resourceDomain'):WSTConf('CONF.resourcePath');
		$data['auctionDesc'] = htmlspecialchars_decode($data['auctionDesc']);
        $auctionDesc = str_replace($resourceDomain.'/upload/','${DOMAIN}/upload/', $data['auctionDesc']);
        $auction->auctionDesc = WSTRichEditorFilter($auctionDesc);
        Db::startTrans();
		try{
			$auction->save();
	        //拍卖描述图片
	        $desc = $this->where('auctionId',$grouponId)->value('auctionDesc');
	        WSTEditorImageRocord(0, $grouponId, $desc, $auctionDesc);
	        Db::commit();
	...
	public function complateAuctionRefund($obj){
		...
		try{
			...
			if(!empty($amoney)){
				...
				$userId = $amoney['userId'];
				$auctionId = $amoney['auctionId'];
				$auctionName = $amoney['goodsName'];
				$auctionMoney = $amoney['cautionMoney'];
				$payType = $amoney['payType'];
				//创建用户资金流水记录
				$lm = [];
				$lm['targetType'] = 0;
				$lm['targetId'] = $userId;
				$lm['dataId'] = $auctionId;
				$lm['dataSrc'] = 'auction';
				$lm['remark'] = '拍卖活动【'.$auctionName.'】退款¥'.$auctionMoney.'元到会员钱包。';
				$lm['moneyType'] = 1;
				$lm['money'] = $auctionMoney;
				$lm['payType'] = $payType;
				$lm['tradeNo'] = '';
				$lm['createTime'] = date('Y-m-d H:i:s');
				model('common/LogMoneys')->create($lm);
				//创建一条支出流水记录
				$lm = [];
				$lm['targetType'] = 0;
				$lm['targetId'] = $userId;
				$lm['dataId'] = $auctionId;
				$lm['dataSrc'] = 'auction';
				$lm['remark'] = '拍卖活动【'.$auctionName.'】退款¥'.$auctionMoney.'元从会员钱包原支付渠道退回。';
				$lm['moneyType'] = 0;
				$lm['money'] = $auctionMoney;
				$lm['payType'] = $payType;
