function queryByPage(p){
	$('#list').html('<tr><td colspan="11"><img src="'+WST.conf.RESOURCE_PATH+'/wstmart/home/view/default/img/loading.gif">正在加载数据...</td></tr>');
	var params = {};
	params = {};
...
function changeDeliverType(n,index,obj){
	...
	if(n==1){	//自提订单需要先验证手机
		...
		if(bindPhone==0){
			WST.confirm({content:'自提订单需要绑定手机，点击确定前往绑定',yes:function(index){
				location.href=WST.U('home/users/editPhone');
			}});
		}
	}
}
function submitOrder(){
	...
	$.post(WST.AU('auction://auction/submit'),params,function(data,textStatus){
		layer.close(load);   
		var json = WST.toJson(data);
	    if(json.status==1){
