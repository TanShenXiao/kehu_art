...
function onSwitch(obj,n){
	...
	if($('#bindPhone').val()==0){
		type = $(obj).children('.ui-icon-push').attr('mode');
		if(type==1)
			$('#msgBindPhone').css('display','block');
		else
			$('#msgBindPhone').css('display','none');
	}
}
/* 选择是否需要发票 */
...
function submitOrder(){
	...
	$.post(WST.AU('auction://auction/submit'),param,function(data,textStatus){
		var json = WST.toJson(data);
	    WST.noload();
	    if(json.status==1){
...
function addAddress(type,id){
	...
	location.href = WST.AU('auction://useraddress/moindex','type='+type+'&addressId='+id+'&auctionId='+auctionId);
}
function goGoods(id){
