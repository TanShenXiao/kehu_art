...
class Goods extends Controller{
    ...
    public function welists(){
        ...
        $data['goodscats'] = $gModel->getGoodsCats();
        $ads =  model("common/Tags")->listAds("weapp-ads-bargain",99,86400);
        $data['ads'] = $ads;
     ...
     * 砍价列表
     */
    public function weBargainlists(){
        $m = new Bargains();
        $rs = $m->pageQuery();
        if(!empty($rs['data'])){
            foreach ($rs['data'] as $key =>$v){
    ...
    public function wedetail(){
        ...
        if(!empty($goods)){
            ...
            // 砍价规则
            $goods['article'] = htmlspecialchars_decode($goods['article']);
            $rule = '/<img src="\/(upload.*?)"/';
            preg_match_all($rule, $goods['goodsDesc'], $images);
            foreach($images[0] as $k=>$v){
                $goods['article'] = str_replace('/'.$images[1][$k], Request::root().'/'.WSTConf("CONF.goodsLogo") . "\"  data-echo=\"".Request::root()."/".WSTImg($images[1][$k],3), $goods['article']);
            }
            ...
            $bargainUserId = (int)base64_decode(input('bargainUserId'));
            $rs['bargainUserId'] = input('bargainUserId');
            //砍价个人信息
            $user = ['userType'=>0];
            $user['bargainType'] = 0;
            if($userId>0 || $bargainUserId>0){
            ...
            $rs['signType'] = ($userId>0)?1:0;
            $rs['user'] = $user;
            //分享信息
            $rs['bargainUserId'] = base64_encode($userIds);
            $rs["nowTime"] = date("Y-m-d H:i:s");
            return jsonReturn('success',1,$rs);
        }
    }
}