...
class Bargains extends Base{
    ...
    public function install(){
    	Db::startTrans();
		try{
			$hooks = ['wechatDocumentUserIndexTools','adminDocumentHookSummary'];
	...
	public function addMobileBtn(){
		...
		$data["btnName"] = "全民砍价";
		$data["btnSrc"] = 1;
		$data["btnUrl"] = "addon/bargain-goods-wxlists.html";
		...
		if(WSTDatas('ADS_TYPE',4)){
			...
			$data["btnName"] = "全民砍价";
			$data["btnSrc"] = 3;
			$data["btnUrl"] = "wst://Bargain";
			$data["btnImg"] = "addons/bargain/view/app/img/bargain.png";
			$data["addonsName"] = "Bargain";
			$data["btnSort"] = 7;
			Db::name('mobile_btns')->insert($data);
		}
        ...
        if(WSTDatas('ADS_TYPE',5)){
            ...
            $data["btnName"] = "全民砍价";
            $data["btnSrc"] = 2;
            $data["btnUrl"] = "/addons/package/pages/bargain/goods/list";
            $data["btnImg"] = "addons/bargain/view/images/bargain.png";
            $data["addonsName"] = "Bargain";
            $data["btnSort"] = 7;
            Db::name('mobile_btns')->insert($data);
        }
	 ...
	 * 第一刀
	 */
	function firstKnife($uId=0){
		$userId = ($uId==0)?(int)session('WST_USER.userId'):$uId;
		$isWepp = (int)input('isWeapp');
		...
		if(empty($rs)){
			...
			if(false !== $result){
				$data = [];
				if($isWepp==1){
                    $user = Db::name('users')->where(['userId'=>$uId])->field('weOpenId,userName,userPhoto')->find();
				    $openId = $user['weOpenId'];
                    ...
                    $userName = session('WST_USER.userName')?session('WST_USER.userName'):session('WST_USER.loginName');
                    $userPhoto = session('WST_USER.userPhoto')?session('WST_USER.userPhoto'):'';
                }
	...
	function addKnife($uId=0){
		...
		if($isWeapp==1){
            if($signType==0)return WSTReturn('请先登录小程序，参与活动');
        }else{
	...
	public function getAuditCount(){
		return $this->where(['dataFlag'=>1,'bargainStatus'=>0])->count();
	}
}
