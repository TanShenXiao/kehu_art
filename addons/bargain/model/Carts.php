...
class Carts extends Base{
	...
	public function getCarts($uId=0){
		...
		->where($where)
		->field('s.userId,s.shopId,s.shopName,g.goodsId,s.shopQQ,shopWangWang,b.goodsName,bs.currPrice shopPrice,b.floorPrice,b.goodsStock,b.orderNum,g.goodsImg,g.goodsCatId,g.goodsType,gs.specIds,gs.id goodsSpecId,b.startTime,b.endTime,g.isFreeShipping,s.isInvoice,
	 ...
	 * 虚拟商品下单
	 */
	public function submitByVirtual($carts,$orderSrc = 0,$uId=0){
		$addressId = 0;
		...
		try{
			...
			if(false !== $result){
				...
				Db::name('bargains')->where('bargainId',$carts['bargainId'])->update([
					'orderNum'=>Db::raw('orderNum+'.$goods['cartNum']),
					'goodsStock'=>Db::raw('goodsStock-'.$goods['cartNum'])
				]);
				//添加订单号
	...
	public function submitByEntity($carts,$orderSrc = 0,$uId=0){
		...
		try{
			...
			if(false !== $result){
				...
				$orderGoods['getScoreVal'] = WSTMoneyGiftScore($orderGoods['goodsPrice']*$goods['cartNum']);
				// 获得的积分可抵扣金额
