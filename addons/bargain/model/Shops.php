...
class Shops extends Base{
	...
	public function pageQuery($sId=0){
		...
		$bargainStatus=input('bargainStatus');
		$shopId =($sId==0)?(int)session('WST_USER.shopId'):$sId;
		$where = [];
		$where[] = ['b.shopId','=',$shopId];
		$where[] = ['b.dataFlag','=',1];
		if($bargainStatus !='')$where[]=['bargainStatus','=',$bargainStatus];
		if($goodsName !='')$where[] = ['b.goodsName|g.goodsSn','like','%'.$goodsName.'%'];
        $page =  Db::name('bargains')->alias('b')->join('__GOODS__ g','g.goodsId=b.goodsId and g.isSale=1 and g.dataFlag=1')
                      ->where($where)
                      ->field('b.*,g.goodsImg,g.goodsSn')
    ...
    public function searchGoods(){
    	...
    	$rs = Db::name('goods')->where($where)->field('goodsName,goodsSeoKeywords,goodsSeoDesc,goodsId,marketPrice,shopPrice')->select();
	...
	public function getById($id,$sId=0){
		$where = [];
		$where['b.shopId'] =($sId==0)?(int)session('WST_USER.shopId'):$sId;
		$where['b.bargainId'] = $id;
		$where['b.dataFlag'] = 1;
		$where['b.dataFlag'] = 1;
		return Db::name('bargains')->alias('b')
		         ->join('__GOODS__ g','g.goodsId=b.goodsId','left')
		         ->where($where)->field('g.goodsImg,g.marketPrice,b.*')
		         ->find();
	}
	...
	public function add($sId=0){
		$data =input('post.');
		if(isset($data['tokenId']))unset($data['tokenId']);
		$shopId =($sId==0)?(int)session('WST_USER.shopId'):$sId;
	...
	public function edit($sId=0){
		...
		$bargainId = (int)$data['bargainId'];
		unset($data['bargainId']);
		$result = Db::name('bargains')->where(['bargainId'=>$bargainId,'shopId'=>$shopId])->update($data);
		if(false !== $result){
    ...
    public function pageByOrders(){
		...
		$where[] = ['shopId','=',$shopId];
		$where[] = ['dataFlag','=',1];
		$where[] = ['orderCode','=','bargain'];
		$where[] = ['orderCodeTargetId','=',(int)input('bargainId')];
		if($orderNo!=''){
