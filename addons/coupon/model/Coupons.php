...
class Coupons extends Base{
    ...
    public function installMenu(){
		...
		try{
			$hooks = ['homeDocumentGoodsPropDetail','mobileDocumentGoodsPropDetail','wechatDocumentGoodsPropDetail',
			          ...
			          'homeDocumentSettlementShopSummary','mobileDocumentUserIndexTerm','wechatDocumentUserIndexTerm','offlineBeforeOrderPay','offlineAfterOrderPayComplete','offlineAfterQueryCarts'];
			...
			//商家中心
			Db::name('home_menus')->insert(["parentId"=>77,"menuName"=>"优惠券","menuUrl"=>"/addon/coupon-shops-index","menuOtherUrl"=>"/addon/coupon-shops-edit,/addon/coupon-shops-pageQuery,/addon/coupons-shops-toEdit,/addon/coupon-shops-del,/addon/coupon-shops-coupons,/addon/shops/pageQueryByCoupons,/addon/shops/toStat,/addon/shops/stat","menuType"=>1,"isShow"=>1,"menuSort"=>1,"dataFlag"=>1,"createTime"=>$now,"menuMark"=>"coupon"]);
			$this->addMobileBtn();
			installSql("coupon");
			Db::commit();
			return true;
	...
	public function addMobileBtn(){
		...
		$data["btnName"] = "领券中心";
		$data["btnSrc"] = 1;
		$data["btnUrl"] = "addon/coupon-coupons-wxindex.html";
		...
		$data["btnName"] = "领券中心";
		$data["btnSrc"] = 2;
		$data["btnUrl"] = "/addons/package/pages/coupon/coupon";
		...
		if(WSTDatas('ADS_TYPE',4)){
			...
			$data["btnImg"] = "addons/coupon/view/app/img/coupon.png";
			$data["addonsName"] = "Coupon";
			$data["btnSort"] = 5;
			Db::name('mobile_btns')->insert($data);
		}
    ...
    public function searchGoods($sId=0){
    	...
    	$rs = Db::name('goods')->where($where)->field('goodsName,goodsId,marketPrice,shopPrice,goodsSn,goodsImg')->select();
    ...
    public function add($sId=0){
		...
		if(strtotime($data['startDate']) >= strtotime($data['endDate']))return WSTReturn('开始时间必须小于结束时间');
        $goods = [];
    ...
    public function edit($sId=0){
        ...
        if(empty($couponNum))return WSTReturn('无效的记录');
    ...
    public function offlineAfterOrderPayComplete($params){
        ...
        if(isset($extraJson["promotionItems"]["1"])){
            ...
            if(!empty($couponUser)){
                ...
                if($coupon['useCondition']==1){
                    $data['userCouponJson'] = json_encode(['text'=>'满'.$coupon['useMoney']."减".$coupon['couponValue'],'money'=>$coupon['couponValue']]);
                }else{
	                $data['userCouponJson'] = json_encode(['text'=>"优惠券¥".$coupon['couponValue'],'money'=>$coupon['couponValue']]);
	            }
        ...
        if($params['carts']['carts']){
            foreach ($params['carts']['carts'] as $key => $v) {
                ...
                foreach ($coupons as $ckey => $coupon) {
                	...
                	$item["itemTxt"] = "满¥".$coupon["useMoney"]."减¥" .$coupon["couponValue"];
