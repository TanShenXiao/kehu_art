$(document).ready(function() {
    ...
    decoration.show_dialog_module_html = function($dialog, content) {
	    ...
	    decoration.editor = KindEditor.create('#module_html_editor', {
	    	...
	    	uploadJson : WST.U('home/shops/editorUpload'),
        	...
        	WST.upload({
                  ...
                  duplicate:false,
	       	  	  ...
	       	  	  callback:function(f){
	       	  		  ...
	       	  		  if(json.status==1){
	      	 	          $('#div_module_slide_image').html('<img src="' +  WST.conf.RESOURCE_PATH+"/"+json.savePath+json.name + '" data-image-name="' +  WST.conf.RESOURCE_PATH+"/"+json.savePath+json.name + '">');
	       	  		  }
	       		  },
        	...
        	WST.upload({
                  ...
                  duplicate:false,
	       	  	  ...
	       	  	  callback:function(f){
	       	  		  var json = WST.toJson(f);
	       	  		  if(json.status==1){
	      	 	          $('#div_module_hot_area_image').html('<img src="' + WST.conf.RESOURCE_PATH+"/"+json.savePath+json.name + '" data-image-name="' + WST.conf.RESOURCE_PATH+"/"+json.savePath+json.name + '">');
	      	              decoration.$hot_area_image = $('#div_module_hot_area_image').find('img');
	    	 ...
	    	 WST.upload({
                  ...
                  duplicate:false,
		   	  	  ...
		   	  	  callback:function(f){
		   	  		  var json = WST.toJson(f);
		   	  		  if(json.status==1){
		   	              $('#img_background_image').attr('src', WST.conf.RESOURCE_PATH+"/"+json.savePath+json.name);
		   	              $('#txt_background_image').val(json.savePath+json.name);
    ...
    $('#btn_module_goods_search').on('click', function() {
    	...
    	param.goodsName = $("#searchGoodsName").val();
