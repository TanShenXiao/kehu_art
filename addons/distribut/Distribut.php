...
class Distribut extends Addons{
    ...
    public $info = [
        'name' => 'Distribut',   // 插件标识
        'title' => '分销商品',  // 插件名称
        'description' => 'WSTMart 分销插件，更好帮助推广商城！',    // 插件简介
        'status' => 0,  // 状态
        'author' => 'WSTMart',
        'version' => '2.1.0'
    ];
     ...
     * 编辑商品页加分销设置【shop】
     */
    public function shopDocumentShopEditGoods($params){
    	$m = new DM();
    	...
    	if($conf["isDistribut"]==1){
    		...
    		return $this->fetch('view/shop/goods_edit');
    	}
    }
    ...
    public function beforeEditOrderMoney($params){
        $m = new DM();
        $m->checkEditOrderMoney($params);
    }
     ...
     */
    public function beforeMakeRefundData($params){
        $m = new DM();
        $m->changeDistributMoney($params);
    }
     ...
     * @return [type]         [description]
     */
    public function beforeShopSettlement($params){
        $m = new DM();
        $m->dmoneySettlement($params);
    }
