...
use wstmart\common\model\GoodsCats as CatM;
use wstmart\common\model\Areas as AreaM;
use wstmart\common\model\Brands as BrandM;
use wstmart\common\model\Attributes as AttM;
...
class Goods extends Controller{
    ...
    public function glist(){
    	...
    	$m = new CatM();
        $goodsCatIds = [];
        if($catId>0)$goodsCatIds = $m->getParentIs($catId);
        reset($goodsCatIds);
    	//填充参数
    	$data = [];
    	$data['catId'] = $catId;
