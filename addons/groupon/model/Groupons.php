...
class Groupons extends Base{
    ...
    public function installMenu(){
    	Db::startTrans();
		try{
			$hooks = ['afterEditGoods','afterCancelOrder','adminDocumentHookSummary'];
	...
	public function pageQueryByShop($sId=0){
		...
		$where[] = ['g.dataFlag','=',1];
		$where[] = ['g.isSale','=',1];
		if($goodsName !='')$where[] = ['gu.goodsName|g.goodsSn', 'like', '%'.$goodsName.'%'];
        ...
        $page =  $this->alias('gu')
                      ...
                      ->field('g.goodsSn,g.goodsName,g.goodsImg,gu.*')
	...
	public function getById($id,$sId=0){
		...
		$where['gu.dataFlag'] = 1;
		$where['g.dataFlag'] = 1;
		return $this->alias('gu')->join('__GOODS__ g','g.goodsId=gu.goodsId','left')->where($where)->field('g.marketPrice,g.shopPrice,gu.*')->find();
	}
	...
	public function pageQuery(){
		...
		$page = Db::name('groupons')->alias('gu')->join('__GOODS__ g','gu.goodsId=g.goodsId','inner')
		          ...
		          ->where($where)
		          ->field('g.goodsImg,g.marketPrice,gu.*')
	...
	public function getCarts(){
		...
		$rs = $this->alias('gu')->join('__GOODS__ g','gu.goodsId=g.goodsId','inner')
		           ...
		           ->where($where)
		           ->field('s.userId,s.shopId,s.shopName,g.goodsId,s.shopQQ,shopWangWang,gu.goodsName,gu.grouponPrice shopPrice,gu.grouponNum goodsStock,gu.orderNum,g.goodsImg,g.goodsCatId,g.goodsType,gs.specIds,gs.id goodsSpecId,gu.startTime,gu.endTime,g.isFreeShipping,s.isInvoice,
	...
	public function getCartMoney($uId=0){
		...
		if($isUseScore==1){
			...
			$moneyToScore = WSTScoreToMoney($data['totalMoney'],true);
	...
	public function pageQueryByAdmin($grouponStatus){
        ...
        $page =  $this->alias('gu')->join('__GOODS__ g','g.goodsId=gu.goodsId and g.isSale=1 and g.dataFlag=1','inner')
                      ->join('__SHOPS__ s','s.shopId=gu.shopId','left')
                      ->where($where)->order('gu.createTime desc')->field('g.goodsSn,gu.*,g.goodsImg,s.shopId,s.shopName')
	...
	*/
	public function illegal(){
		...
		$rs = $this->alias('gu')
		           ...
		           ->where('grouponId',$id)
		           ->field('gu.grouponId,g.shopId,s.userId,g.goodsSn,gu.grouponStatus,g.goodsId')->find();
	...
	*/
	public function allow(){	
		...
		$rs = $this->alias('gu')
		           ...
		           ->join('__SHOPS__ s','g.shopId=s.shopId','left')
		           ->where('grouponId',$id)
		           ->field('gu.grouponId,gu.goodsName,g.shopId,s.userId,g.goodsSn,gu.grouponStatus,g.goodsId')->find();
		if((int)$rs['grouponId']==0)return WSTReturn("无效的商品");
