...
function getCartMoney(){
	...
	WST.load('正在计算价格...');
	if(goodsType==0){
		$.post(WST.AU('integral://carts/getCartMoney'),params,function(data,textStatus){
			WST.noload();
			var json = WST.toJson(data);
			if(json.status==1){
			    json = json.data;
...
function submitOrder(){
	...
	$.post(WST.AU('integral://carts/submit'),param,function(data,textStatus){
		var json = WST.toJson(data);
	    WST.noload();
	    if(json.status==1){
...
function addAddress(type,id){
	...
	location.href = WST.AU('integral://useraddress/moindex','type='+type+'&addressId='+id);
}
function goGoods(id){
    location.href=WST.AU('integral://goods/modetail','id='+id);
}
