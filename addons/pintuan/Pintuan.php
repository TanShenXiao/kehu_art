...
class Pintuan extends Addons{
    ...
    public $info = [
        'name' => 'Pintuan',   // 插件标识
        'title' => '拼团',  // 插件名称
        'description' => '拼团插件<br/><font color="red">【仅微信端，需计划任务支持!!】</font>',    // 插件简介
        'status' => 0,  // 状态
        'author' => 'WSTMart',
        'version' => '1.0.0'
    ];
    ...
    public function adminDocumentHookSummary(){
        if(WSTGrant('PINTUAN_PTHD_00')){
            $m = new DM();
            $num = $m->getAuditCount();
            echo '<li>
                        <div class="icon">
                            <span><a class="menuItem" href="'.Url('/addon/pintuan-goods-pageByAdmin').'">'.$num.'</a></span>
                        </div>
                        <div class="txt">
                            <a class="menuItem" href="'.Url('/addon/pintuan-goods-pageByAdmin').'">
                                <p>拼团审核</p>
                            </a>
                        </div>
                  </li>';
