...
class Pintuans extends Base{
    ...
    public function installMenu(){
    	Db::startTrans();
		try{
			$hooks = ['beforeCancelOrder','wechatDocumentUserIndexTools','adminDocumentHookSummary'];
			...
			//商家中心
			Db::name('home_menus')->insert(["parentId"=>77,"menuName"=>"拼团","menuUrl"=>"/addon/pintuan-shops-pintuan","menuOtherUrl"=>"/addon/pintuan-shops-pintuan,/addon/pintuan-shops-pageQuery,/addon/pintuan-shops-searchGoods,/addon/pintuan-shops-edit,/addon/pintuan-shops-toEdit,/addon/pintuan-shops-del,/addon/pintuan-shops-unsale,/addon/pintuan-shops-orders,/addon/pintuan-shops-pageQueryByGoods","menuType"=>1,"isShow"=>1,"menuSort"=>1,"dataFlag"=>1,"createTime"=>$now,"menuMark"=>"pintuan"]);
			$this->addMobileBtn();
			installSql("pintuan");
			Db::commit();
			return true;
	...
	public function addMobileBtn(){
		...
		$data["btnName"] = "拼团";
		$data["btnSrc"] = 1;
		$data["btnUrl"] = "addon/pintuan-goods-wxlists.html";
		...
		if(WSTDatas('ADS_TYPE',5)){
	        ...
	        $data["btnName"] = "拼团";
	        $data["btnSrc"] = 2;
	        $data["btnUrl"] = "/addons/package/pages/pintuan/goods/list";
	        $data["btnImg"] = "addons/pintuan/view/images/pintuan.png";
	        $data["addonsName"] = "Pintuan";
	        $data["btnSort"] = 5;
	        Db::name('mobile_btns')->insert($data);
	    }
    ...
    public function searchGoods($sId=0){
    	...
    	$rs = Db::name('goods')->where($where)->field('goodsName,goodsSeoKeywords,goodsSeoDesc,goodsId,marketPrice,shopPrice,goodsType')->select();
	...
	public function getById($id,$sId=0){
		...
		$where['p.dataFlag'] = 1;
		$where['g.dataFlag'] = 1;
		return $this->alias('p')->join('__GOODS__ g','g.goodsId=p.goodsId','left')->where($where)->field('g.marketPrice,g.shopPrice,p.*')->find();
	}
	...
	public function add($sId=0){
		...
		$goodsName = input('goodsName');
		if($goodsName=='')return WSTReturn('请输入商品名称');
		$tuanPrice = (float)$data['tuanPrice'];
		if($tuanPrice<=0)return WSTReturn('拼团价格必须大于0');
		if((int)$data['tuanNum']<=0)return WSTReturn('拼团数量必须大于0');
		if($tuanPrice>=$goods['shopPrice'])return WSTReturn('拼团价格应小于店铺原价');
	...
	public function getCarts($uId=0){
		...
		$rs = $this->alias('p')->join('__GOODS__ g','p.goodsId=g.goodsId','inner')
		           ...
		           ->where($where)
		           ->field('s.userId,s.shopId,s.shopName,g.goodsId,s.shopQQ,shopWangWang,g.goodsName,p.tuanPrice,g.shopPrice,p.goodsNum ,p.saleNum,g.goodsImg,g.goodsCatId,g.goodsType,gs.specIds,gs.id goodsSpecId,g.isFreeShipping,s.isInvoice,
	...
	public function getCartMoney($uId=0){
		...
		//计算最大可用积分
		$maxScoreMoney = $data['totalMoney'];
		$maxScore = WSTScoreToMoney($data['totalMoney'],true);
		//最大可用积分不能大于用户积分
        $userId = $uId==0?(int)session('WST_USER.userId'):$uId;
	...
	public function submitByVirtual($carts,$orderSrc = 0,$uId){
		...
		$cnt = Db::name("pintuan_users")->where(['userId'=>$userId,"tuanNo"=>$tuanNo,"dataFlag"=>1])->count();
		if($cnt>0){
			return WSTReturn("您已参与该团，不能重复参团");
		}
		...
		try{
			...
			if($shopIsInvoice==1 && $isInvoice==1){
				...
				$puser['invoiceJson'] = model('common/invoices')->getInviceInfo((int)input('invoiceId'),$userId);// 发票信息
			    $puser['invoiceClient'] = $invoiceClient;
			}else{
				$puser['isInvoice'] = 0;
				$puser['invoiceJson'] = "";// 发票信息
				$puser['invoiceClient'] = "";
			}
			...
			if($puId>0){
				...
				$this->where('tuanId',$carts['tuanId'])->setDec('goodsNum',$goods['cartNum']);
	...
	public function submitByEntity($carts,$orderSrc = 0,$uId=0){
		...
		try{
			...
			if($carts['isFreeShipping']){
			    ...
			    $puser['deliverMoney'] = ($deliverType==1)?0:WSTOrderFreight($carts['shopId'],$address['areaId2'],$carts);
			}
			$puser['totalMoney'] = $puser['goodsMoney']+$puser['deliverMoney'];
            //积分支付-计算分配积分和金额
            $puser['scoreMoney'] = 0;
	...
	public function getTuanInfo($tuanNo, $uId=0){
        ...
        $cdate = $tuan['createTime'];
     	...
     	$tuan["economizePrice"] = WSTBCMoney($goods["shopPrice"],-$tuan["tuanPrice"]);
     	...
     	$pself = Db::name('pintuan_users pu')->join("__USERS__ u","pu.userId=u.userId")
									...
									->field("pu.id,pu.tuanStatus,pu.orderNo")
									->find();
					
		$tuan["pself"] = $pself;
		$tuan['pkey'] = '';
		if(!empty($pself) && $pself['tuanStatus']==0){
			$tuan['pkey'] = WSTBase64urlEncode($pself["orderNo"]);
		}
     	$pusers = Db::name('pintuan_users pu')->join("__USERS__ u","pu.userId=u.userId")
	...
	*/
	...
	public function pinTuanSuccess($tuanNo){
		...
		if($tuanStatus==1 && $needNum==0){
			...
			for($i=0,$j=count($pusers);$i<$j;$i++){
				...
				$orderGoodscommission = $data["commissionFee"];
	...
	public function pulist($uId=0){
        ...
        if(count($page['data'])>0){
        	...
        	foreach($page['data'] as $key =>$v){
        		...
        		if($v['tuanStatus']==0){
        			$page['data'][$key]['pkey'] = WSTBase64urlEncode($v["orderNo"]);
        		}
        	}
        }
        $page['status'] = 1;
	 ...
	 * 用户取消拼团
	 */
	public function delTuan($userId){
		$id = (int)input('id');
		$data = [];
		$data['tuanStatus'] = 0;
		$data['userId'] = $userId;
	...
	public function getLastTuan(){
		...
		if($newMaxPuId>$maxPuId){
			...
			$data["userPhoto"] = WSTUserPhoto($puser['userPhoto']);
			if($tuanId>0){
	 ...
	 * 获取商品的当前正在拼团的列表
	 */
	public function getPulist($tuanId=0){
		$tuanId = $tuanId>0?$tuanId:(int)input("id");
		$where = ['dataFlag'=>1,'tuanId'=>$tuanId];
		$tuan = $this->where($where)->field("tuanTime,tuanNum")->find();
		if(empty($tuan))return [];
		$pusers = Db::name('pintuan_users pu')->join("__USERS__ u","pu.userId=u.userId")
	...
	public function getAuditCount(){
		return $this->where(['dataFlag'=>1,'tuanStatus'=>0])->count();
	}
	...
	*/
	public function payOverTime(){
		$cfg = $this->getConf("Pintuan");
		$pintuanPaytime = (int)$cfg['pintuanPaytime'];
	 	$pintuanPaytime = ($pintuanPaytime>0)?$pintuanPaytime:15;
	 	$lastDay = date("Y-m-d H:i:s",strtotime("-".$pintuanPaytime." minute"));
	 	$pusers = Db::name('pintuan_users')->where([
				 		['createTime','<',$lastDay],
				 		['tuanStatus','=',-2],
				 		['dataFlag','=',1],
				 		['payType','=',1],
	 	...
	 	if(!empty($pusers)){
	 		$puIds = [];
	 		foreach ($pusers as $okey => $pu){
	 			$puIds[] = $pu['id'];
	 		}
	 		Db::startTrans();
		    try{
		    	//提前锁定订单
		    	Db::name('pintuan_users')->where([['id','in',$puIds]])->update(['tuanStatus'=>-1]);
                foreach ($pusers as $okey => $pu){
                	$tuanId = $pu['tuanId'];
                	$goodsNum =  $pu['goodsNum'];
                	//未付款状态则直接退回积分
                    if($pu['useScore']>0){
                    	$score = [];
						$score['userId'] = $pu['userId'];
						$score['score'] = $pu['useScore'];
						$score['dataSrc'] = 1;
						$score['dataId'] = $pu['id'];
						$score['dataRemarks'] = "取消拼团订单【".$pu['orderNo']."】，退回积分".$pu['useScore']."个";
						$score['scoreType'] = 1;
						model('common/UserScores')->add($score);
                    }
					Db::name('pintuans')->where('tuanId',$tuanId)->setInc('goodsNum',$goodsNum);
                    //发送消息
	                ...
	                if( $tpl['tplContent']!='' && $tpl['status']=='1'){
	                    ...
	                    $replace = [$pu['orderNo']];
	                    //发送一条用户信息
					    WSTSendMsg($pu['userId'],str_replace($find,$replace,$tpl['tplContent']),['from'=>1,'dataId'=>$pu['id']]);
	                }
