...
function initGrid1(p){
    ...
    var cols = [
            ...
            {title:'参团所需人数', name:'tuanNum', width: 70},
            ...
            {title:'有效时间（小时）', name:'tuanTime', width: 80},
    ...
    mmg1 = $('.mmg1').mmGrid({height: h-126,indexCol: true, indexColWidth:50, cols: cols,method:'POST',
        url: WST.AU('pintuan://goods/pageQueryByAdmin'), fullWidthRows: true, autoLoad: false,
        plugins: [
            $('#pg1').mmPaginator({})
        ]
    });
...
function initGrid2(p){
    ...
    mmg2 = $('.mmg2').mmGrid({height: h-126,indexCol: true, indexColWidth:50, cols: cols,method:'POST',
        url: WST.AU('pintuan://goods/pageAuditQueryByAdmin'), fullWidthRows: true, autoLoad: false,
        plugins: [
            $('#pg2').mmPaginator({})
        ]
    });
