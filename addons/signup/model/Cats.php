...
class Cats extends Base{
	...
	public function pageQueryCatsByAdmin(){
        ...
        $page =  Db::name('signup_cats')->order('startDate desc')->where($where)
        ...
        if(count($page['data'])>0){
        	...
        	foreach($page['data'] as $key =>$v){
				...
				$limit = $v['signupLimit'];
				if($limit != 0){
					$rs = Db::name('signup_lists')->where('catId',$v['catId'])->count();
					if($rs>=$limit)
						$page['data'][$key]['status'] = -1;
				}
        	}
	...
	public function pageQueryCats(){
        ...
        $page =  Db::name('signup_cats')->order('catSort,startDate')->where($where)
	...
	public function getCatById($id){
		...
		$where['catId'] = (int)$id;
		$where['dataFlag'] = 1;
		$rs = Db::name('signup_cats')->where($where)->find();
        $today = date('Y-m-d');
		...
		if(!empty($rs)){
			...
			$rs['afterSignup'] = htmlspecialchars_decode($rs['afterSignup']);
		}
		return $rs;
	}
	...
	public function getCats(){
        ...
        $rs =  Db::name('signup_cats')->order('catSort,startDate')->where($where)->select();
        if(count($rs)>0){
        	$today = date('Y-m-d');
        	foreach($rs as $key =>$v){
				$rs[$key]['catDesc'] = htmlspecialchars_decode($v['catDesc']);
				$rs[$key]['afterSignup'] = htmlspecialchars_decode($v['afterSignup']);
        		if(strtotime($v['startDate'])<=strtotime($today) && strtotime($v['endDate'])>=strtotime($today)){
        			$rs[$key]['status'] = 1; 
        		}else if(strtotime($v['startDate'])>strtotime($today)){
                    $rs[$key]['status'] = 0; 
				...
				$limit = $v['signupLimit'];
				if($limit != 0){
					$lr = Db::name('signup_lists')->where('catId',$v['catId'])->count();
					if($lr>=$limit)
						$rs[$key]['status'] = -1;
				}
        	}
