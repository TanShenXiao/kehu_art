...
class Ucenter extends Addons{
     ...
     * 修改邮箱前【shop】
     */
    public function shopControllerUsersEditEmail($params){
        if(!$this->ucOpen)return;
        //获取用户信息
        $userId = (int)session('WST_USER.userId');
        $m = new \wstmart\common\model\Users();
        ...
        if($data["userFrom"]==100){
            ...
            if($data['userEmail']){
                exit($m->fetch('../../../addons/ucenter/view/shop/users/user_edit_email'));
            }else{
                exit($m->fetch('../../../addons/ucenter/view/shop/users/user_email'));
            }
        }
