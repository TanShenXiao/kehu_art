...
class AcpService {
	...
	public static function updateEncryptCert(&$params)
	{
		...
		if($certType === "01"){
			...
			if (CertUtil::getEncryptCertId() != $certInfo['serialNumber']) {
				$newFileName = getBackupFileName(SDKConfig::$encryptCertPath);
				// 将原证书备份重命名
				if(!copy(SDKConfig::$encryptCertPath, $newFileName)){
					$logger->LogError ('原证书备份失败');
					return -1;
				}
				// 更新证书
				if(!file_put_contents(SDKConfig::$encryptCertPath, $strCert)){
	...
	static function sign(&$params) {
		if($params['signMethod']=='01')	{
			return AcpService::signByCertInfo($params, SDKConfig::$signCertPath, SDKConfig::$signCertPwd);
		} else {
			return AcpService::signBySecureKey($params, SDKConfig::$secureKey);
		} 
	}
	...
	static function validate($params) {
		...
		if($params['signMethod']=='01')
		{
			...
			$isSuccess = AcpService::validateBySecureKey($params, SDKConfig::$secureKey);
		} 
		return $isSuccess;
	}
...
eot;
	...
	static function getEncryptCertId() {
		$cert_path=SDKConfig::$encryptCertPath;
	...
	static function decryptData($data, $cert_path=null, $cert_pwd=null) {
		...
		if( $cert_path == null ) {
			$cert_path = SDKConfig::$signCertPath;
			$cert_pwd = SDKConfig::$signCertPwd;
		}
