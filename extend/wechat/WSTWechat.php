...
class WSTWechat{
			 ...
			 if($key==0){
                 ...
                 switch ($v['msgType']) {
                 	case 'text':$resultStr = sprintf($this->tpl['text'], $postObj->FromUserName, $postObj->ToUserName, time(), $v['content'], 0);
                 		        break;
                 	case 'news':$newC = sprintf($this->tpl['content'], $v['title'], $v['description'], WSTConf('CONF.resourcePath').'/'.$v['picUrl'], $v['url']);
	...
	public function _doText($postObj)
	{
		...
		if($msgType=='text'){
			...
			$newC='';
			for($i=0;$i<$count;++$i){
				$newC .= sprintf($this->tpl['content'], $news[$i]['title'], $news[$i]['description'], WSTConf('CONF.resourcePath').'/'.$news[$i]['picurl'], $news[$i]['url']);
			}
			//将内容输出到新闻模板
     ...
     * 发送小程序模板
     */
    public function sendWeTemplateMessage($data){
