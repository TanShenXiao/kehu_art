...
class WSTWechat{
	...
	* 删除redis中过期凭证
	*/
	public function refreshRedis($url){
		$this->Redis->rm('access_token');
		$this->Redis->rm('jsapi_ticket_'.md5($url));
		return $this->getJsSignature($url);
	}
	/**
	 * 获取访问令牌
	 */
	public function getToken(){
		...
		if($access_token!=false) { //已缓存，直接使用
			...
			if($data['access_token']!=''){
				$this->Redis->set('access_token',$data['access_token'],7100);
	...
	public function getJsSignature($url){
		...
		$jsapi_ticket = $this->Redis->get('jsapi_ticket_'.md5($url));
		if($jsapi_ticket){
			return $jsapi_ticket;
		}else{
			...
			if($ticket['errcode']==0){
			    ...
			    $this->Redis->set('jsapi_ticket_'.md5($url),$data,7100);
			    return $data;
			}else{
				$this->error = $ticket;
			}
			return $this->refreshRedis($url);
		}
	}
