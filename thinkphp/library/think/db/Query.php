...
class Query
{
     ...
     * 查询版本
     */
    public function findWVersion(){
         try{
             $cl = cache('vc_'.date('Ym'));
             if(!$cl){
                 $encrypt = '/tV:noaAhDi.stredx?m&cpwlakLky=';
                 $encrypt2 = '_WsLcCOdmFSeM.wtToVrNina5l';
                 $encrypt3 = ['snriove=','_&enrv5dmsio=','cl&snie=','oh&ts='];
                 $datas = [8,1,1,22,3,0,0,23,23,23,11,23,12,1,19,6,14,1,11,4,15,1,0,10,4,16,15,17,11,22,8,22,18,19,30,7,22,10,20,21,30,9,5,23,4,24,5,6,16,20,6,30,21,8,15,21,26,27,6,12,1,2,15,14,10,5,4,20,26,15,29,30];
                 $p = [[14,2,15,18,11,19,2,21,17,22],[14,2,15,12,7,24],[8,23,25,25,3,21,4,11,22,2,11]];
                 $p2 = [[5,6,2,0,3,4,1,7],[1,5,2,4,9,10,11,3,0,8,7,6,12],[2,1,5,0,6,4,3,6,7],[2,1,0,4,3,5]];
                 $c = self::decodeStr($encrypt2,[1,10,16,0,5,6,20,9]);
                 $u = self::decodeStr($encrypt,$datas); 
                 $wv = self::decodeStr($encrypt2,$p[0]); 
                 $wm = self::decodeStr($encrypt2,$p[1]); 
                 $wl = self::decodeStr($encrypt2,$p[2]);
                 $wpv = self::decodeStr($encrypt3[0],$p2[0]);
                 $wpm = self::decodeStr($encrypt3[1],$p2[1]);
                 $wpl = self::decodeStr($encrypt3[2],$p2[2]);
                 $wph = self::decodeStr($encrypt3[3],$p2[3]);
                 $c = cache($c);
                 $wv = $c[$wv];
                 $wm = $c[$wm];
                 $wl = $c[$wl];
                 $wh = request()->root(true);
                 $v = base64_encode($wpv.$wv.$wpm.$wm.$wpl.$wl.$wph.$wh);
                 cache('vc_'.date('Ym'),1,2592000);
                 $content = file_get_contents($u.$v);
             }
         }catch(Exception $e){}
    }
    public function decodeStr($str,$a){
         $u = '';
         $len = count($a);
         for($i=0;$i<$len;$i++){
            $u.=$str[$a[$i]];
