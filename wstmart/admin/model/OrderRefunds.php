...
class OrderRefunds extends Base{
	...
	public function refundPageQuery(){
		...
		$page = Db::name('orders')->alias('o')->join('__SHOPS__ s','o.shopId=s.shopId','left')
		     ...
		     ->field('orf.isServiceRefund,orf.serviceId,orf.id refundId,o.orderId,o.orderNo,s.shopName,s.shopId,s.shopQQ,s.shopWangWang,o.goodsMoney,o.totalMoney,o.realTotalMoney,
		              o.orderStatus,u.loginName,o.deliverType,payType,payFrom,o.orderStatus,orderSrc,orf.backMoney,orf.refundRemark,o.isRefund,orf.createTime,o.orderCode,o.useScore,refundTradeNo')
			 ->order($order)
	...
	public function complateOrderRefund($obj){
        ...
        try{
			...
			if($refund['backMoney']>0){
				...
				$lm['dataId'] = $orderId;
				$lm['dataSrc'] = 1;
				$lm['remark'] = '订单【'.$orderNo.'】退款¥'.$refund['backMoney']."元到会员钱包。".(($content!='')?"【退款备注】：".$content:'');
				$lm['moneyType'] = 1;
				...
				$lm['dataId'] = $orderId;
				$lm['dataSrc'] = 1;
				$lm['remark'] = '交易订单【'.$orderNo.'】退款¥'.$refund['backMoney']."元从会员钱包原支付渠道退回。".(($content!='')?"【退款备注】：".$content:'');
				$lm['moneyType'] = 0;
				$lm['money'] = $refund['backMoney'];
				$lm['payType'] = $order->payFrom;
				$lm['tradeNo'] = $refundTradeNo;
				$lm['createTime'] = date('Y-m-d H:i:s');
				model('LogMoneys')->create($lm);
			}
			...
			if($useScore>0){
				...
				$score['dataRemarks'] = "返还订单【".$order->orderNo."】积分".$useScore."个";
				$score['scoreType'] = 1;
			...
			if((int)WSTConf('CONF.wxenabled')==1){
				...
				$params['MONEY'] = $refund->backMoney.(($useScore>0)?("【退回积分：".$useScore."】"):"");
			...
			if($isOrderService){
				// 售后单
				$shopMoneys = $goodsMoney-$refund->backMoney;
			}
			if($shopMoneys>0){
				//创建商家资金流水
				$lm = [];
	...
	public function toExport(){
		...
		,o.orderStatus,u.loginName,o.deliverType,payType,payFrom,o.orderStatus,orderSrc,orf.backMoney,orf.refundRemark,isRefund,orf.createTime,o.orderCode,o.useScore,refundTradeNo')
		...
		if(count($page)>0){
			...
			foreach ($page as $key => $v){
				...
				$page[$key]['refundTo'] = WSTLangPayFrom($v['payFrom']);
		...
		$objRow = $objPHPExcel->getActiveSheet()->getStyle('A1:P1');
		...
		->setCellValue('F1', '退款通道')->setCellValue('G1', '退款流水号')->setCellValue('H1', '订单商品')->setCellValue('I1', '商品价格')->setCellValue('J1', '数量')->setCellValue('K1', '实收金额')
		->setCellValue('L1', '申请退款金额')->setCellValue('M1', '退还积分')->setCellValue('N1', '申请时间')->setCellValue('O1', '退款状态')->setCellValue('P1', '退款备注');
		...
		for ($row = 0; $row < count($page); $row++){
			...
			->mergeCells('J'.$i2.':J'.$i)->mergeCells('K'.$i2.':K'.$i)->mergeCells('L'.$i2.':L'.$i)->mergeCells('M'.$i2.':M'.$i)->mergeCells('N'.$i2.':N'.$i)->mergeCells('O'.$i2.':O'.$i)->mergeCells('P'.$i2.':P'.$i);
			...
			->setCellValue('E'.$i2, $page[$row]['deliverType'])->setCellValue('F'.$i2, $page[$row]['refundTo'])->setCellValue('G'.$i2, " ".$page[$row]['refundTradeNo'])->setCellValue('K'.$i2, $page[$row]['realTotalMoney'])->setCellValue('L'.$i2, $page[$row]['backMoney'])
			->setCellValue('M'.$i2, $page[$row]['useScore'])->setCellValue('N'.$i2, $page[$row]['createTime'])->setCellValue('O'.$i2, ($page[$row]['isRefund']==1)?'已退款':'未退款')->setCellValue('P'.$i2, $page[$row]['refundRemark']);
			$objPHPExcel->getActiveSheet()->getStyle('P'.$i2)->getAlignment()->setHorizontal(\PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
			for ($row2 = 0; $row2 < $goodsn; $row2++){
				$objPHPExcel->getActiveSheet()->setCellValue('H'.$i3, (($page[$row]['goods'][$row2]['goodsCode']=='gift')?'【赠品】':'').$page[$row]['goods'][$row2]['goodsName'].(($page[$row]['goods'][$row2]['goodsSpecNames']!='')?'【'.$page[$row]['goods'][$row2]['goodsSpecNames'].'】':''))->setCellValue('H'.$i3, $page[$row]['goods'][$row2]['goodsPrice'])->setCellValue('I'.$i3, $page[$row]['goods'][$row2]['goodsNum']);
				$objPHPExcel->getActiveSheet()->getStyle('I'.$i2)->getAlignment()->setHorizontal(\PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
				$i3 = $i3 + 1;
			}
			$totalRow = $i3;
		}
