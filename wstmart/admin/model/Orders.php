...
class Orders extends Base{
	...
	public function pageQuery($orderStatus = 10000,$isAppraise = -1){
		...
		$page = $this->alias('o')->join('__USERS__ u','o.userId=u.userId','left')->join('__SHOPS__ s','o.shopId=s.shopId','left')->where($where)
		     ->field('o.orderId,o.orderNo,u.loginName,s.shopName,s.shopId,s.shopQQ,s.shopWangWang,o.goodsMoney,o.totalMoney,o.realTotalMoney,
		              o.orderStatus,o.userName,o.deliverType,payType,payFrom,o.orderStatus,orderSrc,o.createTime,o.orderCode,o.deliverMoney,o.scoreMoney,o.tradeNo')
			 ->order($order)
			 ->paginate(input('limit/d'))->toArray();
	    if(count($page['data'])>0){
	    	 foreach ($page['data'] as $key => $v){
	    	 	 $page['data'][$key]['userName'] = "【".$v['loginName']."】".WSTAnonymous($v['userName']);
	    	 	 $page['data'][$key]['payType'] = WSTLangPayType($v['payType']);
				 $page['data'][$key]['payFrom'] = WSTLangPayFrom($v['payFrom']);
	...
	public function getByView($orderId){
		...
		$orders['userName'] = WSTAnonymous($orders['userName']);
		$orders['userAddress'] = WSTAnonymous($orders['userAddress']);
		$orders['userPhone'] = WSTAnonymous($orders['userPhone']);
		return $orders;
	}
	 ...
	 * 获取用户订单商品列表
	 */
	public function pageQueryOrderGoods($orderStatus = 10000,$isAppraise = -1){
		$where = $this->orderGoodsFilter();
		if($orderStatus!=10000){
			$where[] = ['orderStatus','=',$orderStatus];
		}
		$order = 'o.createTime desc';
		...
		$page = $this->alias('o')->join('__USERS__ u','o.userId=u.userId','left')->join('__SHOPS__ s','o.shopId=s.shopId','left')
			->join('order_goods og','og.orderId=o.orderId','left')->join('goods g','og.goodsId=g.goodsId','left')->join('goods_cats c','g.goodsCatId=c.catId','left')->where($where)
		     ->field('o.orderId,o.orderNo,u.loginName,s.shopName,s.shopId,s.shopQQ,s.shopWangWang,o.goodsMoney,o.totalMoney,o.realTotalMoney,
		              o.orderStatus,u.userName,o.deliverType,payType,payFrom,o.orderStatus,orderSrc,o.createTime,o.orderCode,
					  og.goodsName,og.goodsNum,og.goodsPrice,og.goodsImg,og.goodsType,g.goodsSn,c.catName,g.oriGoodsId,og.goodsNum*og.goodsPrice totalPrice,og.goodsNum*og.goodsPrice*og.commissionRate/100.0 commission,og.commissionRate')
			 ->order($order)
			 ->paginate(input('limit/d'))->toArray();
	    if(count($page['data'])>0){
	    	 foreach ($page['data'] as $key => $v){
	    	 	 ...
	    	 	 $page['data'][$key]['status'] = WSTLangOrderStatus($v['orderStatus']);
				 if($v['goodsType']==1){
					$page['data'][$key]['goodsType'] = '虚拟商品';
				 }else if($v['goodsType']==2){
					$page['data'][$key]['goodsType'] = '保底商品';
				 }else{
					$page['data'][$key]['goodsType'] = '普通商品';
				 }
				 if($v['oriGoodsId']>0){
					$page['data'][$key]['orderCode'] = '保底交易';
				 }else{
					$page['data'][$key]['orderCode'] = WSTOrderModule($v['orderCode']);
				 }
	    	 }
	    }
	    return $page;
	 ...
	 * 获取用户订单资金列表
	 */
	public function pageQueryOrderMoney($orderStatus = 10000,$isAppraise = -1){
		$where = $this->orderMoneyFilter();
		if($orderStatus!=10000){
			$where[] = ['orderStatus','=',$orderStatus];
		}
		$order = 'o.createTime desc';
		...
		$page = $this->alias('o')->join('__USERS__ u','o.userId=u.userId','left')->join('__SHOPS__ s','o.shopId=s.shopId','left')->join('settlements t','o.settlementId=t.settlementId','left')->join('order_refunds r','o.orderId=r.orderId','left')->where($where)
		     ->field('o.orderId,o.orderNo,u.loginName,s.shopName,s.shopId,s.shopQQ,s.shopWangWang,o.goodsMoney,o.totalMoney,o.realTotalMoney,
		              o.orderStatus,o.userName,o.deliverType,payType,payFrom,o.orderStatus,orderSrc,o.createTime,o.orderCode,o.deliverMoney,o.scoreMoney,t.settlementNo,o.tradeType,o.payTime,o.isRefund,r.backMoney,r.refundTime')
			 ->order($order)
			 ->paginate(input('limit/d'))->toArray();
	    if(count($page['data'])>0){
	    	 foreach ($page['data'] as $key => $v){
	    	 	 ...
	    	 	 $page['data'][$key]['payFrom'] = WSTLangPayFrom($v['payFrom']);
				 if($v['tradeType']>0){
					$og = Db::name('order_goods')->alias('og')->join('goods g','og.goodsId=g.goodsId')->field('g.goodsType,g.oriGoodsId')->where('og.orderId',$v['orderId'])->find();
					if(!empty($og)){
						if($og['oriGoodsId']==0){
							$page['data'][$key]['remark'] = '保底购买';
						}else{
							$oriOrder = Db::name('order_goods')->alias('og')->join('orders o','og.orderId=o.orderId')->field('o.orderNo')->where('og.goodsId',$og['oriGoodsId'])->find();
							$page['data'][$key]['remark'] = '出售增值，原订单号【'.$oriOrder['orderNo'].'】';
						}
					}
				 }
	    	 }
	 ...
	 * 导出订单前的筛选
	 */
	public function orderMoneyFilter(){
		$where[] = ['o.dataFlag','=',1];
	...
	public function orderGoodsFilter(){
		$where[] = ['o.dataFlag','=',1];
		$orderStatus = input('orderStatus');
		if($orderStatus!=10000){
			$where[] = ['orderStatus','=',$orderStatus];
	...
	public function toExportOrderMoney(){
		...
		if(count($page)>0){
			foreach ($page as $key => $v){
				...
				$page[$key]['status'] = WSTLangOrderStatus($v['orderStatus']);
	    	 	 ...
	    	 	 $page[$key]['payFrom'] = WSTLangPayFrom($v['payFrom']);
				 $page[$key]['remark'] = '';
				 if($v['tradeType']>0){
					$og = Db::name('order_goods')->alias('og')->join('goods g','og.goodsId=g.goodsId')->field('g.goodsType,g.oriGoodsId')->where('og.orderId',$v['orderId'])->find();
					if(!empty($og)){
						if($og['oriGoodsId']==0){
							$page[$key]['remark'] = '保底购买';
						}else{
							$oriOrder = Db::name('order_goods')->alias('og')->join('orders o','og.orderId=o.orderId')->field('o.orderNo')->where('og.goodsId',$og['oriGoodsId'])->find();
							$page[$key]['remark'] = '出售增值，原订单号【'.$oriOrder['orderNo'].'】';
						}
					}
				 }
				if($v['isRefund']==1)	$page[$key]['isRefund'] = '是';
				else $page[$key]['isRefund'] = '否';
			}
		}
		require Env::get('root_path') . 'extend/phpexcel/PHPExcel/IOFactory.php';
		$objPHPExcel = new \PHPExcel();
		...
		->setSubject($name)//题目
		->setDescription($name)//描述
		->setKeywords("订单收款信息")//关键字
		->setCategory("ordermoney file");//种类
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A1', '交易时间')->setCellValue('B1', '支付时间')->setCellValue('C1', '订单编号')->setCellValue('D1', '订单成交类型')->setCellValue('E1', '买家')
		->setCellValue('F1', '店铺')->setCellValue('G1', '商品金额')->setCellValue('H1', '运费')->setCellValue('I1', '订单总金额')->setCellValue('J1', '积分')
		->setCellValue('K1', '实付金额')->setCellValue('L1', '支付通道')->setCellValue('M1', '订单状态')->setCellValue('N1', '是否退款')->setCellValue('O1', '退款金额')->setCellValue('P1', '退款时间')->setCellValue('Q1', '备注')->setCellValue('R1', '商家结算号');
		...
		for ($row = 0; $row < count($page); $row++){
			...
			->mergeCells('Q'.$i2.':Q'.$i)->mergeCells('R'.$i2.':R'.$i)->mergeCells('S'.$i2.':S'.$i)->mergeCells('T'.$i2.':T'.$i)->mergeCells('U'.$i2.':U'.$i)->mergeCells('V'.$i2.':V'.$i)->mergeCells('W'.$i2.':W'.$i)
			->mergeCells('X'.$i2.':X'.$i)->mergeCells('Y'.$i2.':Y'.$i);
			$objPHPExcel->getActiveSheet()->setCellValue('A'.$i2, $page[$row]['createTime'])->setCellValue('B'.$i2, $page[$row]['payTime'])->setCellValue('C'.$i2, $page[$row]['orderNo'])->setCellValue('D'.$i2, $page[$row]['orderCodeTitle'])
			->setCellValue('E'.$i2, $page[$row]['userName'])->setCellValue('F'.$i2, $page[$row]['shopName'])->setCellValue('G'.$i2, $page[$row]['goodsMoney'])->setCellValue('H'.$i2, $page[$row]['deliverMoney'])
			->setCellValue('I'.$i2, $page[$row]['totalMoney'])->setCellValue('J'.$i2, " ".$page[$row]['scoreMoney'])->setCellValue('K'.$i2, $page[$row]['realTotalMoney'])->setCellValue('L'.$i2, $page[$row]['payFrom'])
			->setCellValue('M'.$i2, $page[$row]['status'])->setCellValue('N'.$i2, $page[$row]['isRefund'])->setCellValue('O'.$i2, $page[$row]['backMoney'])->setCellValue('P'.$i2, $page[$row]['refundTime'])->setCellValue('Q'.$i2, $page[$row]['remark'])->setCellValue('R'.$i2, $page[$row]['settlementNo']);
			$objPHPExcel->getActiveSheet()->getStyle('F'.$i2)->getAlignment()->setHorizontal(\PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
			$objPHPExcel->getActiveSheet()->getStyle('X'.$i2)->getAlignment()->setHorizontal(\PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
		}
	...
	public function toExportOrderGoods(){
	    ...
	    if(count($page)>0){
	    	 foreach ($page as $key => $v){
	    	 	 ...
	    	 	 $page[$key]['status'] = WSTLangOrderStatus($v['orderStatus']);
				 if($v['goodsType']==1){
					$page[$key]['goodsType'] = '虚拟商品';
				 }else if($v['goodsType']==2){
					$page[$key]['goodsType'] = '保底商品';
				 }else{
					$page[$key]['goodsType'] = '普通商品';
				 }
				 if($v['oriGoodsId']>0){
					$page[$key]['orderCode'] = '保底交易';
				 }else{
					$page[$key]['orderCode'] = WSTOrderModule($v['orderCode']);
				 }
	    	 }
	    }
		...
		->setSubject($name)//题目
		->setDescription($name)//描述
		->setKeywords("订单商品详情")//关键字
		->setCategory("ordergoods file");//种类
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A1', '下单时间')->setCellValue('B1', '订单编号')->setCellValue('C1', '订单类型')->setCellValue('D1', '商品类型')->setCellValue('E1', '商品类别')
		->setCellValue('F1', '买家')->setCellValue('G1', '店铺')->setCellValue('H1', '商品编码')->setCellValue('I1', '商品名称')->setCellValue('J1', '数量')
		->setCellValue('K1', '单价')->setCellValue('L1', '总价')->setCellValue('M1', '支付方式')->setCellValue('N1', '佣金比率')->setCellValue('O1', '佣金')->setCellValue('P1', '订单状态');
		$objPHPExcel->getActiveSheet()->getStyle('A1:Y1')->applyFromArray($styleArray);
		...
		for ($row = 0; $row < count($page); $row++){
			...
			->mergeCells('Q'.$i2.':Q'.$i)->mergeCells('R'.$i2.':R'.$i)->mergeCells('S'.$i2.':S'.$i)->mergeCells('T'.$i2.':T'.$i)->mergeCells('U'.$i2.':U'.$i)->mergeCells('V'.$i2.':V'.$i)->mergeCells('W'.$i2.':W'.$i)
			->mergeCells('X'.$i2.':X'.$i)->mergeCells('Y'.$i2.':Y'.$i);
			$objPHPExcel->getActiveSheet()->setCellValue('A'.$i2, $page[$row]['createTime'])->setCellValue('B'.$i2, $page[$row]['orderNo'])->setCellValue('C'.$i2, $page[$row]['orderCode'])->setCellValue('D'.$i2, $page[$row]['goodsType'])
			->setCellValue('E'.$i2, $page[$row]['catName'])->setCellValue('F'.$i2, $page[$row]['userName'])->setCellValue('G'.$i2, $page[$row]['shopName'])->setCellValue('H'.$i2, ' '.$page[$row]['goodsSn'])
			->setCellValue('I'.$i2, $page[$row]['goodsName'])->setCellValue('J'.$i2, " ".$page[$row]['goodsNum'])->setCellValue('K'.$i2, $page[$row]['goodsPrice'])->setCellValue('L'.$i2, $page[$row]['totalPrice'])
			->setCellValue('M'.$i2, $page[$row]['payType'])->setCellValue('N'.$i2, $page[$row]['commissionRate'])->setCellValue('O'.$i2, $page[$row]['commission'])->setCellValue('P'.$i2, $page[$row]['status']);
			$objPHPExcel->getActiveSheet()->getStyle('F'.$i2)->getAlignment()->setHorizontal(\PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
			$objPHPExcel->getActiveSheet()->getStyle('X'.$i2)->getAlignment()->setHorizontal(\PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
		}
		//输出EXCEL格式
