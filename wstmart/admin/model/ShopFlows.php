...
use wstmart\admin\validate\ShopFlows as validate;
use wstmart\admin\validate\ShopBase as ShopBaseValidate;
use think\Db;
/**
 * ============================================================================
 * WSTMart多用户商城
...
class ShopFlows extends Base{
    ...
    public function del(){
		...
		try{
	        ...
	        if(false !== $result){
                //删除其下的字段
                Db::name('shop_bases')->where(['flowId'=>$id])->update(['dataFlag'=>-1]);
	        	Db::commit();
    ...
    public function getFlowData(){
        $id = (int)input('id');
        $res = Db::name('shop_bases')->where(['flowId'=>$id,'dataFlag'=>1])->select();
        return $res;
    }
    ...
    public function fieldPageQuery(){
        ...
        return Db::name('shop_bases')->where($where)->paginate(input('limit/d'));
    }
    ...
    public function saveField(){
        ...
        try{
            ...
            if($data['id']!=0){
                $old = Db::name('shop_bases')->where(['id'=>$data['id'],'flowId'=>$flowId])->field('fieldName,isShopsTable,isDelete')->find();
                // 如果属于wst_shops表，则不允许修改字段与字段属性
                // 如果属于wst_extras表的基础字段，也不允许修改字段与字段属性
                // 排除以上两种情况，才允许修改字段与字段属性
                if($old['isShopsTable'] == 0 && $old['isDelete'] == 0){
                    // 如果修改了字段名称，则对shop_extras表相应的字段进行改名，否则只修改字段属性
                    if($fieldName != $old['fieldName']){
                        $sql = "alter table `wst_shop_extras` change {$old['fieldName']} {$fieldName} {$dataAttr} {$default}";
                        Db::query($sql);
                    }else{
                        $sql = "alter table `wst_shop_extras` modify {$fieldName} {$dataType}{$dataAttr} {$default}";
                        Db::query($sql);
                    }
                }
                Db::name('shop_bases')->where(['id'=>$data['id'],'flowId'=>$flowId])->update($array);
            }else{
                // 获取shop_extras表的所有字段名称
                $fields = Db::query('show columns from wst_shop_extras');
                $fieldArray = [];
                foreach($fields as $v){
                    $fieldArray[] = $v['Field'];
                }
                // 判断添加的字段是否存在shop_extras表，不存在则新增字段，存在则返回
                if(!in_array($fieldName,$fieldArray)){
                    $sql = "alter table `wst_shop_extras` add column {$fieldName} {$dataType}{$dataAttr} {$default}";
                    Db::query($sql);
                }else{
                ...
                $array['dataFlag'] = 1;
                $array['isShopsTable'] = 0;
                $array['isDelete'] = 1;
                Db::name('shop_bases')->insert($array);
            }
    ...
    public function getFieldById($id){
        $field =  Db::name('shop_bases')->where(['id'=>$id])->find();
    ...
    public function delField(){
        ...
        Db::startTrans();
        try{
            $result = Db::name('shop_bases')->where(['id'=>$id])->update(['dataFlag'=>-1]);
            // 删除shop_extras表相应的字段
            $fieldName = Db::name('shop_bases')->where(['id'=>$id])->value('fieldName');
            $fields = Db::query('show columns from wst_shop_extras');
            $fieldArray = [];
            foreach($fields as $v){
                $fieldArray[] = $v['Field'];
            ...
            if(in_array($fieldName,$fieldArray)){
                $sql = "alter table `wst_shop_extras` drop column {$fieldName}";
                Db::query($sql);
            }
            if(false !== $result){
                Db::commit();
    ...
    public function getFlowFieldsById($id){
        if($id != -1){
            $rs = Db::name('shop_bases')->where(['flowId'=>$id,'dataFlag'=>1])->order('fieldSort asc,id asc')->select();
            return $rs;
        }else{
            $rs = Db::name('shop_bases')->where(['dataFlag'=>1])->whereNotIn('flowId',[2,3])->order('fieldSort asc,id asc')->select();
            return $rs;
        }
    }
}
