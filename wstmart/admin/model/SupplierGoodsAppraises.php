...
use wstmart\admin\validate\SupplierGoodsAppraises as validate;
use think\Db;
/**
 * ============================================================================
 * WSTMart多用户商城
...
class SupplierGoodsAppraises extends Base{
	...
	public function pageQuery(){
		$where = 'p.supplierId=g.supplierId and gp.goodsId=g.goodsId and o.orderId=gp.orderId  and gp.dataFlag=1';
		...
		if($areaId1>0){
	 	...
	 	if($supplierName!='')
	 		$where.=" and (p.supplierName like '%".$supplierName."%' or p.supplierSn like '%".$supplierName."%')";
	 	if($goodsName!='')
		...
		$rs = $this->alias('gp')->field('gp.*,g.goodsName,g.goodsImg,o.orderNo,u.loginName')
					->join('__SUPPLIER_GOODS__ g ','gp.goodsId=g.goodsId','left') 
		         	->join('__SUPPLIER_ORDERS__ o','gp.orderId=o.orderId','left')
		         	->join('__USERS__ u','u.userId=gp.userId','left')
		         	->join('__SUPPLIERS__ p','p.supplierId=gp.supplierId','left')
		         	->where($where)
		         	->order($order)
		         	->order('id desc')
	...
	public function edit(){
        ...
        try{
	        ...
	        if(false !== $result){
	        	...
	        	$this->statGoodsAppraises($goodsAppraises->goodsId,$goodsAppraises->supplierId);
	        	Db::commit();
	 ...
	 * 重新统计商品
	 */
	public function statGoodsAppraises($goodsId,$supplierId){
        $rs = Db::name('supplier_goods_appraises')->where(['goodsId'=>$goodsId,'isShow'=>1,'dataFlag'=>1])
        ...
        //商家评价
        $rs = Db::name('supplier_goods_appraises')->where(['supplierId'=>$supplierId,'isShow'=>1,'dataFlag'=>1])
        ...
        $data['timeScore'] = (int)$rs['serviceScore'];
        $data['timeUsers'] = (int)$rs['userNum'];
        Db::name('supplier_scores')->where('supplierId',$supplierId)->update($data);       
	}
