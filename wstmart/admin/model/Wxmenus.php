...
class WxMenus extends Base{
	...
	public function synchroWx(){
		...
		if($data){
			...
			$type = array('click'=>1,'view'=>2,'scancode_push'=>3,'scancode_waitmsg'=>4,'pic_sysphoto'=>5,'pic_photo_or_album'=>6,'pic_weixin'=>7,'location_select'=>8,'media_id'=>9,'view_limited'=>10,'miniprogram'=>11);
			$dataList = [];
    ...
    function makeNewArr($data,$pId){
    	$type = array(1=>'click',2=>'view',3=>'scancode_push',4=>'scancode_waitmsg',5=>'pic_sysphoto',6=>'pic_photo_or_album',7=>'pic_weixin',8=>'location_select',9=>'media_id',10=>'view_limited',11=>'miniprogram');
    	$c=0;
    	$newArr = [];
    	foreach($data as $k=>$v){
    		if($v['parentId']==$pId){
    ...
    public function edit(){
    	...
    	WSTUnset($data,'menuId,dataFlag,createTime,content,menuType');
    	if(!$data['menuName'])return WSTReturn("请输入菜单名称");
