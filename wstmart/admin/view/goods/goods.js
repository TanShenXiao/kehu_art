...
function initSaleGrid(p){
    ...
    var cols = [
            ...
            {title:'交易类型', name:'goodsType', width: 40,sortable:true,renderer: function(val,item,rowIndex){
				var gtype = '在线销售';
				if(item['saleType']==1)	gtype='议价';
				else if(item['saleType']==2)	gtype='仅展示';
				else if(item['goodsType']==2)	gtype='保底交易';
				else if(item['oriGoodsId']>0)	gtype='保底交易-增值';
                return "<span  ><p class='wst-nowrap'>"+gtype+"</p></span>";
            }},
    ...
    mmg = $('.mmg').mmGrid({height: (h-90),indexCol: true, indexColWidth:50, cols: cols,method:'POST',checkCol:true,multiSelect:true,
...
function toBatchPrint(){
	...
	if(rows.length==0){
		 ...
		 WST.msg('选择商品数量过多,请限制在50个商品以内',{icon:2});
		 return;
	}
	var ids = '';
	for(var i=0;i<rows.length;i++){
	...
	window.open(WST.U("admin/goods/goodsPrint",'ids='+ids));
}
...
function initUpload(){
	...
	WST.upload({
  	  ...
  	  callback:function(f){
  		  ...
  		  if(json.status==1){
			  var savePathName = json.savePath+json.thumb;
			  var loading = WST.msg('正在设置，请稍候...', {icon: 16,time:60000});
			  $.post(WST.U('admin/goods/setBkImg'),{imgPathName:savePathName},function(data,textStatus){
				  layer.close(loading);
				  var json = WST.toAdminJson(data);
				  if(json.status=='1'){
						WST.msg("操作成功",{icon:1});
