...
function flowGridInit(p){
    ...
    var cols = [
            ...
            {title:'客户性质', name:'targetType', width: 30},
            {title:'来源', name:'dataSrc', width: 30},
            {title:'订单号', name:'orderSn', width: 50},
            ...
            {title:'备注', name:'remark',width:350},
            {title:'通道', name:'payType',width:50},
            {title:'通道号', name:'tradeNo',width:120},
            {title:'操作', name:'op' ,width:20,renderer: function (val,item,rowindex){
				if(item['dataSrc']=='商品订单'){
					return '<a class="btn btn-blue" href="javascript:tologmoneys(1,'+item['shopId']+')"><i class="fa fa-search"></i>查看</a>';
				}
	        }}
            ];
...
function loadFlowGrid(p){
    p=(p<=1)?1:p;
	mmg3.load({page:1,key:$('#key3').val(),type:$('#type').val(),startDate:$('#startDate').val(),endDate:$('#endDate').val(),tradeNo:$('#tradeNo').val()});
}
...
function toExport1(){
    var params = {};
    params.key = $('#key1').val();
    var box = WST.confirm({content:"您确定要导出记录吗?",yes:function(){
        layer.close(box);
        location.href=WST.U('admin/logmoneys/toExportByUser',params);
    }});
}
function toExport2(){
    var params = {};
    ...
    var box = WST.confirm({content:"您确定要导出记录吗?",yes:function(){
        layer.close(box);
        location.href=WST.U('admin/logmoneys/toExportByShop',params);
    }});
}
function toExport3(){
    var params = {};
    params.key = $('#key3').val();
	params.userType = $('#type').val();
	params.startDate = $('#startDate').val();
	params.endDate = $('#endDate').val();
	params.tradeNo = $('#tradeNo').val();
    var box = WST.confirm({content:"您确定要导出记录吗?",yes:function(){
        layer.close(box);
        location.href=WST.U('admin/logmoneys/toExportFlow',params);
    }});
}