...
function initGrid(p){
    ...
    var cols = [
            {title:'艺术家名', name:'shopName', width: 100},
            {title:'头像', name:'img', width: 60, renderer: function(val,item,rowIndex){
                return "<span class='weixin'><img id='img' onmouseout='toolTip()' onmouseover='toolTip()' style='height:50px;' src='"+WST.conf.RESOURCE_PATH+"/"+item['shopImg']
            	+"'><span class='imged' style='left:45px;' ><img  style='height:200px; width:200px;' src='"+WST.conf.RESOURCE_PATH+"/"+item['shopImg']+"'></span></span>";
            }},
            { title: '是否置顶', name: 'top',isSort: false,width: 10,renderer: function(val,item,rowIndex){
                let name = ''
                    if(val == 1){
                        name = '是'
                    }else{
