...
function initGrid(p){
    ...
    mmg = $('.mmg').mmGrid({height: h-183,indexCol: true,indexColWidth:50,  indexColWidth:50,cols: cols,method:'POST',multiSelect:true,
...
function toExportShopOrder(){
    ...
    var box = WST.confirm({content:"您确定要导出记录吗?",yes:function(){
        layer.close(box);
        location.href=WST.U('admin/settlements/toExportShopOrder',params);
    }});
}