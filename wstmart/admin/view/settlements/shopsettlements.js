...
function initGrid(p){
    ...
    var cols = [
			...
			{title:'结算处理时间', name:'settlementTime',sortable:true},
...
function initGoodsGrid(id){
    ...
    var cols = [
            ...
            {title:'商品规格', name:'goodsSpecNames',width:150, renderer:function(val,item,rowIndex){
                if(WST.blank(val)!=''){
	            	val = val.split('@@_@@');
	                return val.join('，');
	            }
            }},
			{title:'商品类别', name:'catName' ,width:100},
            {title:'商品价格', name:'goodsPrice' ,width:30, renderer:function(val,item,rowIndex){
                return '￥'+val;
            }},
            {title:'购买数量', name:'goodsNum' ,width:30},
            {title:'佣金比率', name:'commissionRate',width:30},
            {title:'佣金金额', name:'commission',width:30}
            ];
...
function toExportDetail(){
    var params = {};
    params.id = $('#settlementId').val();
    var box = WST.confirm({content:"您确定要导出结算详情吗?",yes:function(){
        layer.close(box);
        location.href=WST.U('admin/settlements/toExportDetail',params);
    }});
}
function toExportGoods(){
    var params = {};
    params.id = $('#settlementId').val();
    var box = WST.confirm({content:"您确定要导出结算商品列表吗?",yes:function(){
        layer.close(box);
        location.href=WST.U('admin/settlements/toExportGoods',params);
    }});
}