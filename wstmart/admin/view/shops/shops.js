...
function initGrid(p){
    ...
    var cols = [
            ...
            {title:'店主联系电话', name:'telephone',width: 80,sortable: true},
            {title:'店主QQ', name:'shopQQ',width: 80,sortable: true},
            {title:'店主邮箱', name:'applyLinkEmail',width: 60,sortable: true},
            {title:'店铺地址', name:'shopAddress',width:110 },
			{title:'店铺类型',name:'shopType',width:50,sortable: true,renderer: function (val,item,rowIndex){
				if(item['shopType']==1) return '个人店铺';
				else return '企业店铺';}
			},
			{title:'排序号',name:'shopSort',width:50,sortable: true},
            ...
            {title:'操作', name:'' ,width:150, align:'center', renderer: function(val,item,rowIndex){
	            ...
	            if(WST.GRANT.DPGL_03)h += "<a class='btn btn-red' href='javascript:toDel(" + item['shopId'] + ",1)'><i class='fa fa-trash-o'></i>删除</a> ";
	            return h;
            }}
            ];
...
function initApplyGrid(p){
    ...
    var cols = [
			...
			{title:'店铺类型', name:'shopType',width:100,renderer: function (val,item,rowIndex){
	        	return (item['shopType']==1)?'个人店铺':'企业店铺';
	        }},
            {title:'所属公司', name:'shopCompany',width:100 },
            {title:'申请联系人', name:'applyLinkMan',width:30 },
            {title:'申请联系人电话', name:'applyLinkTel',width:60 },
			{title:'店主QQ', name:'shopQQ',width: 50},
            {title:'店主邮箱', name:'applyLinkEmail',width: 60},
...
function initStopGrid(p){
    ...
    var cols = [
            ...
            {title:'店主QQ', name:'shopQQ',width: 50,},
            {title:'店主邮箱', name:'applyLinkEmail',width: 60,},
            {title:'店铺地址', name:'shopAddress',width:100 },
    ...
    $.post(WST.U('admin/shopapplys/getById'),{id:id},function(data,textStatus){
        ...
        if(json.id){
        	...
        	$('#applyIntention').html(json.applyIntention);
           	var box = WST.open({title:'商家入驻申请',type:1,content:$('#applyBox'),area: ['600px', '500px'],btn: ['确定','取消'],yes:function(){
