...
function initGrid(p){
    ...
    mmg = $('.mmg').mmGrid({height: h-267,indexCol: true,indexColWidth:50, cols: cols,method:'POST',nowrap:true,
        url: WST.U('admin/supplierflows/fieldPageQuery'), fullWidthRows: true, autoLoad: false,remoteSort: true,sortName:'',sortStatus:'desc',
        plugins: [
            $('#pg').mmPaginator({})
        ]
    });
...
function getForEdit(id){
    if(id!=0){
        ...
        $.post(WST.U('admin/supplierflows/getFieldById'),{id:id},function(data,textStatus){
            layer.close(loading);
    ...
    var box =WST.open({title:title,type:1,offset:'0px',content:obj,area: [WST.pageWidth()+'px',WST.pageHeight()+'px'],btn: ['确定','取消'],yes:function(){
        $('#fieldForm').isValid(function(v) {
            if (v) {
                ...
                $.post(WST.U('admin/supplierflows/saveField'),params,function(data,textStatus){
                    layer.close(loading);
                    var json = WST.toAdminJson(data);
                    if(json.status=='1'){
...
function toDel(id){
    var box = WST.confirm({content:"您确定要删除该字段吗?",yes:function(){
        var loading = WST.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
        $.post(WST.U('admin/supplierflows/delField'),{id:id},function(data,textStatus){
            layer.close(loading);
            var json = WST.toAdminJson(data);
            if(json.status=='1'){
                WST.msg("操作成功",{icon:1});
