...
function toView(id,sId){
	location.href=WST.U('admin/supplierorders/view','id='+id+'&serviceId='+sId+'&src=supplierorderrefunds&p='+WST_CURR_PAGE);
}
...
function initRefundGrid(p){
    ...
    var cols = [
            ...
            {title:'供货商', name:'supplierName',sortable: true},
    ...
    mmg = $('.mmg').mmGrid({height: (h-160),indexCol: true, indexColWidth:50, cols: cols,method:'POST',
        url: WST.U('admin/supplierorderrefunds/refundPageQuery'), fullWidthRows: true, autoLoad: false,nowrap:true,
        remoteSort:true ,
        sortName: 'createTime',
        sortStatus: 'desc',
        plugins: [
...
function toRefund(id,sId){
	...
	$.post(WST.U('admin/supplierorderrefunds/toRefund',{id:id,serviceId:sId}),{},function(data){
		layer.close(ll);
...
function showDetail(id){
    parent.showBox({title:'订单详情',type:2,content:WST.U('admin/supplierorders/view',{id:id,from:1}),area: ['1020px', '500px'],btn:['关闭']});
}
function toExport(){
	var params = {};
	...
	var box = WST.confirm({content:"您确定要导出订单吗?",yes:function(){
		layer.close(box);
		location.href=WST.U('admin/supplierorderrefunds/toExport',params);
    }});
}