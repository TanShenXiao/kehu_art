...
function initGrid(p){
    ...
    mmg = $('.mmg').mmGrid({height: h-153,indexCol: true, cols: cols,method:'POST',nowrap:true,
        url: WST.U('admin/wetemplatemsgs/pageQuery'), fullWidthRows: true, autoLoad: false,
        plugins: [
            $('#pg').mmPaginator({})
        ]
    });
    ...
    $('#headTip').WSTTips({width:90,height:35,callback:function(v){
         if(v){
             mmg.resize({height:h-153});
         }else{
             mmg.resize({height:h-95});
         }
    }});
    loadQuery(p);
}
...
function addNewRow(){
	var html = ['<tr id="tr_'+childrenNum+'">',
		'<td><input type="text" style="width:92%;margin-left:10px;" id="fiedlCode_'+childrenNum+'"/></td>',
		'<td><input type="text" style="width:98%;margin-left:10px;" id="fiedlVal_'+childrenNum+'"/></td>',
		'<td><input type="button" value="删除" class="btn btn-danger" onclick="javascript:deleteRow('+childrenNum+')" style="margin-left:10px;"></td>',
		'</tr>'
    ];
...
function toEdit(id){
    location.href = WST.U('admin/wetemplatemsgs/toEdit','id='+id+'&p='+WST_CURR_PAGE);
}
function save(type,p){
	...
	$.post(WST.U('admin/wetemplatemsgs/edit'),params,function(data,textStatus){
	    layer.close(loading);
	    var json = WST.toAdminJson(data);
	    if(json.status=='1'){
	        WST.msg("操作成功",{icon:1});
	        location.href = WST.U('admin/wetemplatemsgs/index','src='+type+'&p='+p);
	    }else{
	        WST.msg(json.msg,{icon:2});
	    }
	});
