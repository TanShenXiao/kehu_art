...
function initGrid(p){
    ...
    var cols = [
            {title:'图片', name:'picUrl', width: 60, renderer: function(val,item,rowIndex){
                return (WST.blank(item['picUrl'],'')!='')?"<img width='60' src='"+WST.conf.RESOURCE_PATH+"/"+item['picUrl']+"'/>":"";
            }},
            ...
            {title:'序号', name:'subscribeSort', width: 50, renderer: function(val,item,rowIndex){
                return '<input size="5" type="text" id="subscribeSort_'+item['id']+'" value="'+item['subscribeSort']+'" onblur="javascript:editSort('+item['id']+',this.value)" style="width:120px;"/>';
            }},
    ...
    mmg = $('.mmg').mmGrid({height: h-89,indexCol: true, cols: cols,method:'POST',
        url: WST.U('admin/wxpassivereplys/pagSubscribeQuery'), fullWidthRows: true, autoLoad: false,
        plugins: [
            $('#pg').mmPaginator({})
        ]
    });
