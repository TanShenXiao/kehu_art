...
function initGrid(p){
    ...
    mmg = $('.mmg').mmGrid({height: h-175,indexCol: true, cols: cols,method:'POST',nowrap:true,
        url: WST.U('admin/wxtemplatemsgs/pageQuery'), fullWidthRows: true, autoLoad: false,
        plugins: [
            $('#pg').mmPaginator({})
        ]
    });
...
function toEdit(id){
    location.href = WST.U('admin/wxtemplatemsgs/toEdit','id='+id+'&p='+WST_CURR_PAGE);
}
function save(type,p){
	...
	$.post(WST.U('admin/wxtemplatemsgs/edit'),params,function(data,textStatus){
	    ...
	    if(json.status=='1'){
	        WST.msg("操作成功",{icon:1});
	        location.href = WST.U('admin/wxtemplatemsgs/index','src='+type+'&p='+p);
	    }else{
	        WST.msg(json.msg,{icon:2});
	    }
	});
