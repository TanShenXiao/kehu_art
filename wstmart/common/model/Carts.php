...
class Carts extends Base{
	...
	public function getQuickCarts($uId=0){
		...
		$rs = Db::name('goods')->alias('g')
		           ...
		           ->where($where)
		           ->field('s.userId,s.shopId,s.shopName,g.goodsId,s.shopQQ,shopWangWang,g.goodsName,g.shopPrice,g.shopPrice defaultShopPrice,g.goodsStock,g.goodsImg,g.goodsCatId,g.isFreeShipping,g.oriGoodsId,g.goodsType,g.saleType,g.isSpec')
		           ->find();
	...
	public function getCarts($isSettlement = false, $uId=0){
		...
		$rs = Db::table($prefix.'carts')
		           ...
		           ->where($where)
		           ->field('c.goodsSpecId,c.cartId,s.userId,s.shopId,s.shopName,s.shopImg,s.isInvoice,s.shopType,s.isTaxAuth,g.goodsId,g.marketPrice,g.shippingFeeType,g.shopExpressId,s.shopQQ,shopWangWang,g.goodsName,g.shopPrice,g.shopPrice defaultShopPrice,g.goodsStock,g.goodsWeight,g.goodsVolume,g.isSpec,gs.specPrice,gs.specStock,gs.specWeight,gs.specVolume,g.goodsImg,c.isCheck,gs.specIds,c.cartNum,g.goodsCatId,g.isFreeShipping,g.goodsType,g.saleType,g.oriGoodsId')
		           ->select();		
		$carts = [];
		$goodsIds = [];
		$goodsTotalNum = 0;
		...
		foreach ($rs as $key =>$v){
			...
			$carts[$v['shopId']]['shopImg'] = $v['shopImg'];
	...
	public function getCartInfo($isSettlement = false,$uId = 0){
		...
		$rs = $this->alias('c')->join('__GOODS__ g','c.goodsId=g.goodsId','inner')
		           ...
		           ->where($where)
		           ->field('c.goodsSpecId,c.cartId,g.goodsId,g.goodsName,g.shopPrice,g.goodsStock,g.isSpec,gs.specPrice,gs.specStock,g.goodsImg,c.isCheck,gs.specIds,c.cartNum,g.saleType')
		           ->select();
		$goodsIds = []; 
		$goodsTotalMoney = 0;
		$goodsTotalNum = 0;
