...
class LogRecord extends Base{
    ...
    public function pageQuery(){
        ...
        if($label!='')$where[] = [' l.label','like',"%".$label."%"];
        return $mrs = Db::name('log_record')->alias('l')->join('__STAFFS__ s',' l.staffId=s.staffId','left')
            //->join('__MENUS__ m',' l.menuId=m.menuId','left')
            ->where($where)
            //->field('l.*,s.staffName,m.menuName')
            ->field('l.*,s.staffName')
            ->order('l.operateTime', 'desc')->paginate(input('limit/d'));
