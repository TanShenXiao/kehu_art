...
class OrderServices extends Base{
    ...
    public function shopSend($sId=0){
        ...
        try{
            ...
            $rs = $this->field('shopExpressType,shopExpressId,shopExpressNo,isShopSend,serviceStatus')->where(['id'=>$serviceId])->update($data);
    ...
    public function shopReceive($sId=0){
        ...
        try{
            ...
            if($data['isShopAccept']==1){
                ...
                $field = array_merge($field, ['shopRejectType','shopRejectOther','shopRejectImg']);
                if(!WSTCheckDatas('ORDER_REJECT',$data['shopRejectType']))return WSTReturn('无效的拒收类型');
                $dataName = $this->getServiceTypeText($data['shopRejectType']);
                // 拒收
    ...
    public function dealRefund($sId=0){
        ...
        try{
            ...
            if($isShopAgree==1){
                ...
                if(count($data['disagreeRemark'])==0)return WSTReturn('请输入不受理原因');
                // 不受理
                $data['isClose'] = 1;
        ...
        if(!empty($rs['data'])){
            ...
            foreach($rs['data'] as $k=>$v){
                $imgs = Db::name('order_goods')->alias('og')
                                               ->join('service_goods sg','sg.goodsId=og.goodsId','inner')
    ...
    public function userReceive($uId=0){
        ...
        try{
            ...
            if($data['isUserAccept']==1){
                ...
                $dataName = $this->getServiceTypeText($data['userRejectType']);
                // 拒收
    ...
    private function refunds($uId=0,  $rs=[]){
        ...
        try{
           unset($data['id']);
           $orders = Db::name('orders')->field('areaId, areaIdPath, userName, userAddress, userPhone, afterSaleEndTime')->where('orderId', $orderId)->find();
           $data['createTime'] = $now;
           // 售后申请状态
