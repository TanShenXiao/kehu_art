...
class Orders extends Base{
	...
	public function quickSubmit($orderSrc = 0, $uId=0){
		...
		try{
			...
			if($goods['goodsType']==2){
				$order['tradeType'] = 1;	//保底第一次交易
				$order['orderCode'] = 'guarantee';
			}
			if($goods['oriGoodsId']>0){
				$order['tradeType'] = 2;	//保底转手
			}
			//创建订单前执行钩子
			...
			if(false !== $result){
				...
				}else if($goods['goodsType']==2){	//保底交易
					//创建订单商品记录
					$orderGgoods = [];
					$orderGoods['orderId'] = $orderId;
					$orderGoods['goodsId'] = $goods['goodsId'];
	...
	public function shopOrdersByPage($orderStatus, $sId=0){
		...
		$page = $this->alias('o')->where($where)->where($condition)
		      ...
		      ->field('o.orderRemarks,o.noticeDeliver,o.orderId,orderNo,goodsMoney,totalMoney,realTotalMoney,orderStatus,deliverType,deliverMoney,isAppraise,isRefund,o.deliverType deliverTypes
		              ,payType,payFrom,userAddress,orderStatus,isPay,isAppraise,userName,orderSrc,o.createTime,orf.id refundId,o.orderCode,o.deliverSmsCode')
			  ->order('o.createTime', 'desc')
			  ->paginate()->toArray();
	    if(count($page['data'])>0){
	    	 $orderIds = [];
		...
		if(!empty($order)){
		    ...
		    try{
				...
				if(false != $result){
					...
					if(1){
						$og = Db::name('order_goods')->field('goodsId')->where('orderId',$orderId)->select();
						foreach($og as $g){
							$gs = Db::name('goods')->field('oriGoodsId')->where('goodsId',$g['goodsId'])->find();
							if($gs['oriGoodsId']!=0){	//保底商品
								Db::name('goods')->where('goodsId',$g['goodsId'])->setField('bdStatus',3);	//设为已增值
							}
						}
					}
					//修改商家未计算订单数
	...
	public function getByView($orderId, $uId=0){
		...
		//获取用户信息
		$user = Db::name('users')->field('loginName,userPhone')->where('userId',$userId)->find();
		$orders['user'] = $user;
		return $orders;
	}
	...
	*/
	...
	public function handleVirtualGoods($orderId){
		$order= Db::name('orders')->alias('o')->join('__SHOPS__ s','o.shopId=s.shopId ','inner')
				->where('orderId',$orderId)->field('orderId,orderNo,o.shopId,s.userId,o.userId ouserId,o.realTotalMoney,o.payFrom,o.tradeType')
				->find();
		//新增订单日志
		$logOrder = [];
	...
	public function complatePay ($obj){
		...
		$orders = model('orders')->where($where)->field('needPay,orderId,orderType,orderNo,shopId,payFrom,realTotalMoney,tradeType,deliverType')->select();
			...
			if($needPay>0 && false != $rs){
				foreach ($orders as $key =>$v){
	                ...
	                if($v['orderType']==1){
						...
						if($v->tradeType==1){	//保底交易
							$o = $this->where(['userId'=>$userId,'orderId'=>$v->orderId,'orderStatus'=>0])
										  ->field('orderId,shopId,orderNo,payType,orderScore,realTotalMoney,commissionFee')->find();
							if(!empty($o)){
								$dt = ['orderStatus'=>2,'receiveTime'=>date('Y-m-d H:i:s')];
								$result = $this->where('orderId',$v['orderId'])->update($dt);
								if(false != $result){
									//确认收货后执行钩子
									hook('afterUserReceive',['orderId'=>$v->orderId]);					
									if(WSTConf('CONF.statementType')==1){
										//修改商家未计算订单数
										$prefix = config('database.prefix');
										$upSql = 'update '.$prefix.'shops set noSettledOrderNum=noSettledOrderNum+1,noSettledOrderFee=noSettledOrderFee-'.$o['commissionFee'].' where shopId='.$o['shopId'];
										Db::execute($upSql);
									}else{
										//即时结算
										model('common/Settlements')->speedySettlement($v->orderId);
									}
								}
							}
						}

						if($v->deliverType==1){	//自提订单发送提货码
							$us = Db::name('users')->field('userPhone')->where('userId',$userId)->find();
							...
							//获取商品信息
							$goods = Db::name('order_goods')->field('goodsName')->where('orderId',$orderId)->select();
							$goodsName = '';
							foreach($goods as $g){
								$goodsName .= $g['goodsName'].'、';
							}
							$goodsName = mb_substr($goodsName,0,-1);
							
							$phoneVerify = rand(100000,999999);
							$tpl = WSTMsgTemplates('PHONE_USER_DELIVERY_VERFIY');
							if((int)WSTConf('CONF.smsOpen')==1 && $tpl['tplContent']!='' && $tpl['status']=='1'){
								$params = ['tpl'=>$tpl,'params'=>['ORDER_NO'=>$v['orderNo'],'MALL_NAME'=>WSTConf("CONF.mallName"),'CODE'=>$phoneVerify,'VERFIY_TIME'=>10,'GOODS_INFO'=>$goodsName,'SHOP_ADDR'=>$shopAddr]];
								$m = new LogSms();
								$rv = $m->sendSMS(0,$userPhone,$params,'delivery',$phoneVerify,$userId,0);
								//if($rv['status']!=1)
								//	return $rv;
							}
							Db::name('orders')->where('orderId',$v['orderId'])->update(['deliverSmsCode'=>$phoneVerify]);
	...
	public function payByWallet($uId=0,$type=0){
		...
		if($isSignup==0){	//订单支付
			$orders = $this->field('orderId,orderNo,orderType,needPay,shopId,payFrom,realTotalMoney,tradeType,deliverType')->where($where)->select();
			...
			try{
				...
				foreach ($orders as $order) {
					...
					if(false != $result){
						...
						//保底交易复制原商品,作为新用户的商品
						$this->copyGoods($order['orderId'],$userId);
						//新增订单日志
						$logOrder = [];
						$logOrder['orderId'] = $order->orderId;
						$logOrder['orderStatus'] = 0;
						...
						if($order->orderType==1){
							...
							if($order->tradeType==1){	//保底交易
								$o = $this->where(['userId'=>$userId,'orderId'=>$order->orderId,'orderStatus'=>0])
											  ->field('orderId,shopId,orderNo,payType,orderScore,realTotalMoney,commissionFee')->find();
								if(!empty($o)){
							...
							if($order->deliverType==1){	//自提订单发送提货码
								$us = Db::name('users')->field('userPhone')->where('userId',$userId)->find();
								$userPhone = $us['userPhone'];
								if(strlen($userPhone)!=11 || substr($userPhone,0,1)!="1"){
									//return WSTReturn('用户手机号错误.',-1);
								...
								$shopAddr = implode('',$addrs).$shop['shopAddress'];
								//获取商品信息
								$goods = Db::name('order_goods')->field('goodsName,goodsNum')->where('orderId',$order->orderId)->select();
								$goodsName = '';
								foreach($goods as $g){
									$goodsName .= $g['goodsName'].'、';
								}
								...
								$tpl = WSTMsgTemplates('PHONE_USER_DELIVERY_VERFIY');
								if((int)WSTConf('CONF.smsOpen')==1 && $tpl['tplContent']!='' && $tpl['status']=='1'){
									$params = ['tpl'=>$tpl,'params'=>['ORDER_NO'=>$order['orderNo'],'MALL_NAME'=>WSTConf("CONF.mallName"),'CODE'=>$phoneVerify,'VERFIY_TIME'=>10,'GOODS_INFO'=>$goodsName,'SHOP_ADDR'=>$shopAddr]];
									$m = new LogSms();
									$rv = $m->sendSMS(0,$userPhone,$params,'delivery',$phoneVerify,$userId,0);
									//if($rv['status']!=1)
									//	return $rv;
								}
								Db::name('orders')->where('orderId',$order['orderId'])->update(['deliverSmsCode'=>$phoneVerify]);
								//return WSTReturn('提货码发送成功。', 1);
							}
							//发送一条商家信息
							$tpl = WSTMsgTemplates('ORDER_HASPAY');
				...
				Db::rollback();
				return WSTReturn('订单支付失败'.$e->getMessage());
			}
		}else if($isSignup == 1){
			$rs = Db::name('signup_lists')->where('signupSn',$orderNo)->find();
			if(empty($rs)){
				return WSTReturn('系统错误,未找到该报名信息.',-1);
			}else if($rs['isPaid']==1){
				return WSTReturn('您已缴过费',-1);
			}
			//判断订单金额是否正确
			$needPay = $rs['signupFee'];
			//获取用户钱包
			$user = model('users')->get(['userId'=>$userId]);
			if($user->payPwd=='')return WSTReturn('您未设置支付密码，请先设置密码',-1);
			if($user->payPwd!=md5($payPwd.$user->loginSecret))return WSTReturn('您的支付密码不正确',-1);
			...
			try{
					//处理订单信息
					$lockCashMoney = ($rechargeMoney>$needPay)?$needPay:$rechargeMoney;
					$data['isPaid'] = 1;
					$data['payTime'] = date('Y-m-d H:i:s');
					$data['payCode'] = 'wallets';
					Db::name('signup_lists')->where('signupSn',$orderNo)->update($data);
						...
						$lm['dataId'] = $rs['listId'];
						$lm['dataSrc'] = 100;
						$lm['remark'] = '报名【'.$orderNo.'】支出¥'.$needPay;
						$lm['moneyType'] = 0;
						$lm['money'] = $needPay;
						$lm['payType'] = 'wallets';
						model('LogMoneys')->add($lm);
				...
				if(strlen($userPhone)==11 && substr($userPhone,0,1)=="1"){	//检测手机号码
					...
					if( $tpl['tplContent']!='' && $tpl['status']=='1'){
						$params = ['tpl'=>$tpl,'params'=>['SIGNUP_TITLE'=>$rs["catName"],'LOGIN_NAME'=>$rspay['loginName']]];
						$m = new LogSms();
						$rv = $m->sendSMS(0,$userPhone,$params,'singupAddList',"",$userId,0);
					}
				}

				return WSTReturn('缴费成功',2);
			}catch (\Exception $e) {
				Db::rollback();
				return WSTReturn('缴费失败');
			}
		}
	}
	...
	*/
	...
	//保底交易复制原商品
	public function copyGoods($orderId,$userId){
		$rs = Db::name('order_goods')->field('goodsId,goodsNum,goodsSpecId,goodsPrice')->where('orderId',$orderId)->select();
		if(empty($rs))
			return ;
		//Db::startTrans();
		//复制订单商品给新用户
		//try{
			foreach($rs as $g){
				$goods = Db::name('goods')->where('goodsId',$g['goodsId'])->find();
				if($goods['goodsType'] != 2){	// 不是保底商品不复制
					continue;
				}
				$goods['ownerId'] = $userId;
				$goods['shopId'] = 0;
				$goods['dataFlag'] = 1;
				$goods['goodsStock'] = $g['goodsNum'];
				$goods['oriGoodsId'] = $g['goodsId'];
				$goods['lastPrice'] = $g['goodsPrice'];
				$goods['createTime'] = date("Y-m-d H:i:s");
				$goods['bdStatus'] = 0;
				$goods['goodsType'] = 0;
				$goods['goodsSn'] = WSTGoodsNo();
				$goods['productNo'] = WSTGoodsNo();
				unset($goods['goodsId']);
				Db::name('goods')->insert($goods);
				// 原商品设为已增值
				Db::name('goods')->where('goodsId',$g['goodsId'])->setField('bdStatus',3);
			}
		//	Db::commit();
		//	return true;
		//}catch(\Exception $e){
	...
	//发送提货码
	public function sendDeliverSms(){
		$orderId = (int)input('post.deliverId');
        $order = $this->where(['isPay'=>1,'orderId'=>$orderId,'orderStatus'=>0])->field('orderId,orderNo,userId,deliverType,shopId')->find();
        if(empty($order))return WSTReturn('操作失败，请检查订单状态是否已改变');
		//自提订单发送验证短信
		$us = Db::name('users')->field('userPhone')->where('userId',$order['userId'])->find();
		$userPhone = $us['userPhone'];
		if(strlen($userPhone)!=11 || substr($userPhone,0,1)!="1"){
			return WSTReturn('用户手机号错误.',-1);
		}
		//获取店铺地址
		$shop = Db::name('shops')->field('areaId,shopAddress')->where('shopId',$order['shopId'])->find();
		$addrs = model('common/areas')->getParentNames($shop['areaId']);
		$shopAddr = implode('',$addrs).$shop['shopAddress'];
		//获取商品信息
		$goods = Db::name('order_goods')->field('goodsName,goodsNum')->where('orderId',$order['orderId'])->select();
		...
		if((int)WSTConf('CONF.smsOpen')==1 && $tpl['tplContent']!='' && $tpl['status']=='1'){
			$params = ['tpl'=>$tpl,'params'=>['ORDER_NO'=>$order->orderNo,'MALL_NAME'=>WSTConf("CONF.mallName"),'CODE'=>$phoneVerify,'VERFIY_TIME'=>10,'GOODS_INFO'=>$goodsName,'SHOP_ADDR'=>$shopAddr]];
			$m = new LogSms();
			$rv = $m->sendSMS(0,$userPhone,$params,'delivery',$phoneVerify,$order['userId'],0);
			if($rv['status']!=1)
				return $rv;
		}
