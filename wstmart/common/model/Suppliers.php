...
class Suppliers extends Base{
    ...
    public function getSupplierInfo($supplierId,$uId = 0){
        ...
        ->field('s.supplierId,s.supplierImg,s.supplierName,s.supplierAddress,s.supplierQQ,s.supplierWangWang,s.supplierTel,s.serviceStartTime,s.longitude,s.latitude,s.serviceEndTime,s.supplierKeeper,mapLevel,s.areaId,s.isInvoice,s.invoiceRemarks,s.supplierNotice,ser.*')
        ->find();
        if(empty($rs))return [];
        //仅仅是为了获取businessLicenceImg而写的，因为businessLicenceImg不排除被删除掉了
        WSTAllow($rs,'supplierId,supplierImg,supplierName,supplierAddress,supplierQQ,supplierWangWang,supplierTel,serviceStartTime,longitude,latitude,serviceEndTime,supplierKeeper,mapLevel,areaId,isInvoice,invoiceRemarks,businessLicenceImg,supplierNotice');
        //评分
