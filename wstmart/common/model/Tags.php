...
class Tags extends Base{
	...
	*/
	public function getGuessLike($catId,$num,$goodsIds=[]){
        ...
        $goods_group = Db::name('goods')
	                   ...
	                   ->group('goodsCatId')
	                   ->column('goodsCatId'); 
	    $goods = Db::name('goods')->field('goodsId,goodsName,goodsImg,goodsSn,goodsStock,saleNum,shopPrice,marketPrice,isSpec,appraiseNum,visitNum,isNew,goodsUnit,thumbsNum,goodsAuthor,saleType')
	    						  ->where($where)
	...
	public function historyByGoods($num){
        ...
        $goods = Db::name('goods')->alias('g')->join('__SHOPS__ s','g.shopId=s.shopId')
                   ->where($where)->field('s.shopName,s.shopId,goodsId,goodsName,goodsImg,goodsSn,goodsStock,saleNum,shopPrice,marketPrice,isSpec,appraiseNum,visitNum,goodsUnit,thumbsNum,goodsAuthor,saleType')
                   ->limit($num)
                   ->orderRaw($orderBy)
                   ->select(); 
        $ids = [];
	...
	public function listByGoods($type,$catId,$num,$cache = 0){
	    ...
	    if(empty($goods) && ($type=='best')){
        	...
        	$goods = Db::name('goods')->alias('g')->join('__SHOPS__ s','g.shopId=s.shopId')
                   ->where($where)->field('g.goodsTips,s.shopName,s.shopId,goodsId,goodsName,goodsImg,goodsSn,goodsStock,saleNum,shopPrice,marketPrice,isSpec,appraiseNum,visitNum,isNew,goodsUnit,thumbsNum,goodsAuthor,saleType')
	...
	*/
	public function listShop($catId,$num,$cache = 0){
        ...
        $where[] = ['r.dataType','=',0];
	    //$where[] = ['r.goodsCatId','=',$catId];
		$where[] = ['b.shopType','=',$catId];
	    $shops = Db::name('shops')->alias('b')->join('__RECOMMENDS__ r','b.shopId=r.dataId','left')
		...
		foreach ($shops as $k => $v) {
			$shops[$k]['shopNum'] = Db::name('goods')->where(['shopId'=>$v['shopId'],'isSale'=>1,'goodsStatus'=>1,'dataFlag'=>1])->count();
		}
        //为空的话就取分类关联的
		...
		if(!empty($shops)){
			foreach($shops as $k=>$v) {
				$goodsCats = Db::name('cat_shops')->alias('cs')->join('__GOODS_CATS__ gc','cs.catId=gc.catId and gc.dataFlag=1','left')
					->where('shopId',$v['shopId'])->field('cs.shopId,gc.catName')->select();
				$shops[$k]['catshops'] = '';
				foreach($goodsCats as $c) {
					if ($shops[$k]['catshops']) {
						if ($c['catName']) {
							$shops[$k]['catshops'] .=  ','.$c['catName'];
						}
					} else {
						$shops[$k]['catshops'] =  $c['catName'];
					}
	...
	public function listShopCats($parentId=0,$num,$shopId = 0,$cache = 0){
		...
		cache('TAG_SHOP_CATS_'.$shopId."_".$parentId."_".$num."_".$cache,$data,$cache);
		return $data;
	}
	...
	public function listShopGoods($type,$shopId,$cat = 0,$num = 0,$cache = 0){
		...
		//if($cacheData)return $cacheData;
	    $types = ['recom'=>'isRecom','new'=>'isNew','hot'=>'isHot','best'=>'isBest'];
	    $order = ['recom'=>'thumbsNum desc,saleNum desc,goodsId asc','new'=>'saleTime desc,goodsId asc','hot'=>'saleNum desc,goodsId asc','best'=>'saleNum desc,goodsId asc','all'=>'thumbsNum desc,saleNum desc,goodsId asc'];
        ...
        $goods = Db::name('goods')
                   ->where($where)->field('goodsId,goodsName,goodsImg,goodsSn,goodsStock,saleNum,shopPrice,marketPrice,isSpec,appraiseNum,visitNum,goodsAuthor,saleType,thumbsNum')
                   ->order($order[$type])->limit($num)->select();       
        $ids = [];
        foreach($goods as $key =>$v){
        	if($v['isSpec']==1)$ids[] = $v['goodsId'];
        ...
        cache('TAG_SHOP_GOODS_'.$type."_".$shopId."_".$cat."_".$num."_".$cache,$goods,$cache);
        return $goods;
	}
	...
	*/
	public function listShopFloorGoods($catId,$shopId,$num,$cache = 0){
		...
		$goods = Db::name('goods')
                   ->where($where)->field('goodsId,goodsName,goodsImg,goodsSn,goodsStock,saleNum,shopPrice,marketPrice,isSpec,appraiseNum,visitNum,saleType,thumbsNum,goodsAuthor')
                   ->limit($num)->select();
