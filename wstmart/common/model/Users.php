...
class Users extends Base{
    ...
    public function checkLogin($loginSrc = 0){
    	...
    	if(!empty($rs)){
			$userId = $rs['userId'];
			$errTimes = $rs['errTimes'];
			if($errTimes>=5){
				$lastErrTime = strtotime($rs['lastErrTime']);
				$now = strtotime(date('Y-m-d H:i:s'));
				$diff = $now - $lastErrTime;
				if($diff < 60*60)
					return WSTReturn("输入错误次数过多，请1小时后再试！");
			}
            if($rs['loginPwd']!=md5($loginPwd.$rs['loginSecret'])){
            	$errTimes++;
				$this->where(["userId"=>$userId])->update(["errTimes"=>$errTimes, "lastErrTime"=>date('Y-m-d H:i:s')]);
				$remain = 5 - $errTimes;
				if($remain == 0) {
					return WSTReturn("输入错误次数过多，请1小时后再试！");
				}else if($remain < 4) {
					return WSTReturn("用户名或密码错误，您还有" . $remain . "次机会！");
				}
    		...
    		$ip = request()->ip();
    		$update = [];
    		$update = ["lastTime"=>date('Y-m-d H:i:s'),"lastIP"=>$ip,"errTimes"=>0];
    		...
    		if(WSTConf('CONF.signScoreSwitch')==1){
    			$rs['signScoreTime'] = 0;
    			$userscores = Db::name('user_scores')->where([["userId",'=',$userId],["dataSrc",'=',5],['createTime','like',date('Y-m').'%']])->order('createTime desc')->find();
