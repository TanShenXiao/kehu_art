...
class Articles extends Base{
	...
	*/
	public function getHelpById(){
		...
		$rs = $this->alias('a')->join('__ARTICLE_CATS__ ac','a.catId=ac.catId','inner')->where('ac.parentId=7 and  a.dataFlag=1 and a.isShow=1 and a.articleId='.$id)->cache(true)->find();
	...
	*/
	public function nList(){
		...
		$rs = $this->alias('a')
			  ->join('__ARTICLE_CATS__ ac','a.catId=ac.catId','inner')
			  ->field('a.*')
			  ->where(['a.catId'=>$catId,'a.dataFlag'=>1,'a.isShow'=>1,'ac.dataFlag'=>1,'ac.isShow'=>1])->order('a.createTime desc')
			  ->where('ac.parentId','<>',7)
			  ->cache(true)
			  ->paginate();
		return $rs;
	...
	* 获取咨询中中心所有文章
	*/
	public function getArticles($catId = '402'){
		// 获取咨询中心下的所有分类id
	...
	public function getArticlesByCat($catId){
		...
		$rs = $this->alias('a')
			  ...
			  ->where([['a.catId','in',$ids],['ac.parentId','<>',7]])
			  ->distinct(true)
			  ->cache(true)
			  ->select();
	    $data = [];
