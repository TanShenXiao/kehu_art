...
class Shops extends CShops{
    ...
    public function pageQuery($pagesize){
    	...
    	foreach ($page['data'] as $key =>$v){
    		...
    		//商品列表
    		$goods = Db::name('goods')->where(['dataFlag'=> 1,'goodsStatus'=>1,'isSale'=>1,'shopId'=> $v["shopId"]])->field('goodsId,goodsName,shopPrice,goodsImg')->limit(3)->order('saleTime desc')->select();
    ...
    * 首页店铺街列表
    */
    public function indexShopQuery($type=0,$num=8){
        ...
        if(!$cacheData){
            $cacheData = $this->alias('s')
                ...
                ->where(['r.goodsCatId'=>0,'s.shopStatus'=>1,'s.dataFlag'=>1,'r.dataSrc'=>1,'r.dataType'=>0,'s.shopType'=>$type])
                ->field('s.shopId,s.shopName,s.shopAddress,sc.shopStreetImg,s.shopImg,s.shopType')->order('r.dataSort asc')->limit($num)->select();
