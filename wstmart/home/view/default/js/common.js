...
WST.searchIpt = function(){
	...
	$(".j-search-goods span").click(function(){
		$("#search-type").val($(this).attr("data"));
		if($(this).attr("data")==1){
			$("#j-search-artist").attr("class", "j-search-select");
			$("#j-search-artwork").attr("class", "j-search-unselect");
			$("#j-search-author").attr("class", "j-search-unselect");
			$('#search-ipt').attr('placeholder',$('#adsShopWordsSearch').val());
		}else if($(this).attr("data")==2){
			$("#j-search-artist").attr("class", "j-search-unselect");
			$("#j-search-artwork").attr("class", "j-search-unselect");
			$("#j-search-author").attr("class", "j-search-select");
			$('#search-ipt').attr('placeholder',$('#adsAuthorWordsSearch').val());
		}else{
			$("#j-search-artist").attr("class", "j-search-unselect");
			$("#j-search-artwork").attr("class", "j-search-select");
			$("#j-search-author").attr("class", "j-search-unselect");
...
WST.search = function(){
	if($("#search-type").val()==1){
		...
		WST.authorSearch($.trim($('#search-ipt').val()));
	}else{
...
WST.goodsSearch = function(v){
	location.href = WST.U('home/goods/search',{'keyword':v,'keytype':0},true);
}
WST.authorSearch = function(v){
	location.href = WST.U('home/goods/search',{'keyword':v,'keytype':2},true);
}
...
/**记录点赞数**/
WST.recordThumb = function(goodsId,userId,thumbsNum,shopOwnerId){
	if(window.conf.IS_LOGIN==0){
		WST.loginWindow();
		return;
	}
	if(userId > 0){
		if(userId==shopOwnerId){
			WST.confirm({content:"不能给自己的商品点赞"});
			return;
		}
		$.post(WST.U('home/goods/recordThumb'),{goodsId:goodsId,userId:userId},function(data){
			if('success'==data) {
				$('#dz').addClass('dz-red');
				$('#dz').removeClass('dz');
				$('#dzs').addClass('dzs-red');
				$('#dzs').removeClass('dzs');
				var num = parseInt($('#thumbsNum').val())+1;
				$('#dzs').html(num);
				$('#thumbsNum').val(num);
			}else if('fail'==data){
				WST.confirm({content:"今日点赞次数已达上限"});
			}else{
				WST.confirm({content:"系统错误"});
			}
		});
	}else{
		WST.confirm({content:"请登录后点赞"});
	}
}
