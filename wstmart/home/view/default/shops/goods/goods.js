...
function addSpecImg(opts){
	...
	html.push('<tr>',
	            ...
	            (opts.specImg)?'<img height="25"  width="25" id="specImg_'+opts.catId+'_'+specNum+'" src="'+WST.conf.RESOURCE_PATH+"/"+opts.specImg+'" v="'+opts.specImg+'"/>':"",
	...
	WST.upload({
		  ...
		  cat:opts.catId,
	  	  ...
	  	  callback:function(f){
	  		  var json = WST.toJson(f);
	  		  if(json.status==1){
	  			$('#uploadMsg_'+this.cat+"_"+this.num).html('<img id="specImg_'+this.cat+"_"+this.num+'" v="'+json.savePath+json.thumb+'" src="'+WST.conf.RESOURCE_PATH+"/"+json.savePath+json.thumb+'" height="25"  width="25"/>');
	  		  }
		  },
		  progress:function(rate){
...
function illegalByPage(p){
	...
	$.post(WST.U('home/goods/illegalByPage'),params,function(data,textStatus){
	    ...
	    if(json.status==1 && json.data){
	       	...
	       	laytpl(gettpl).render(json.data, function(html){
	       		$('#list2').html(html);
	       		$('.j-lazyGoodsImg').lazyload({ effect: "fadeIn",failurelimit : 10,skip_invisible : false,threshold: 200,placeholder:window.conf.RESOURCE_PATH+'/'+WST.conf.GOODS_LOGO});
		    });
	       	if(json.last_page>1){
	       		laypage({
		        	 cont: 'pager2', 
