...
function storeByPage(p){
	...
	params.page = p;
	$.post(WST.U('home/goods/myGoodsByPage'),params,function(data,textStatus){
	    var json = WST.toJson(data);
	    if(json.status==1 && json.data){
...
function toSale(){
	...
	var c = WST.confirm({content:'您确定要出售增值吗?',yes:function(){
		layer.close(c);
		var params = {};
		params.ids = ids;
		$.post(WST.U('home/goods/priceySale'), params, function(data,textStatus){
			var json = WST.toJson(data);
			if(json.status=='1'){
				layer.alert('操作成功,商品已转入您的店铺,请进入店铺修改并上架.',function(index){
					layer.close(index);
				...
				WST.msg('您需要先开设店铺或者登录店铺。', {icon: 5},function(){
					location.href = WST.U('home/shops/join','type=1');
				});
			}else{
				WST.msg('操作失败!', {icon: 5});
			}
...
function toBackSale(){
	...
	WST.confirm({content:'您确定要保底回购吗?',yes:function(){
		var params = {};
		params.ids = ids;
		$.post(WST.U('home/goods/backSale'), params, function(data,textStatus){
			var json = WST.toJson(data);
			if(json.status=='1'){
				layer.alert('操作成功,商品已进入保底回购状态,请仔细阅读回购事项并留下联系方式.',function(index){
					layer.close(index);
					storeByPage(0);
				   //location.href = WST.U('home/goods/store');
				});
			}else if(json.status=='-1'){
				WST.msg(json.msg, {icon: 5});
			}else{
...
function toDelivery(){
	...
	WST.confirm({content:'您确定要提货吗?',yes:function(){
		var params = {};
		params.ids = ids;
		$.post(WST.U('home/goods/bdDelivery'), params, function(data,textStatus){
			var json = WST.toJson(data);
			if(json.status=='1'){
				layer.alert('操作成功,商品已登记提货,请凭借验证码和注册手机号到线下服务中心提货.',function(index){
					layer.close(index);
					storeByPage(0);
				   //location.href = WST.U('home/goods/store');
				});
...
var isUse = false;
var index2 = null;
function resendSms(goodsId){
	if(isSend )return;
	var c = WST.confirm({content:'您确定要重发验证码吗?',yes:function(){
		layer.close(c);
		isSend = true;
		if(window.conf.SMS_VERFY=='1'){
			...
			index2 = layer.open({
				...
				yes: function(index, layero){
					isSend = true;
					smsVerfy = $.trim($('#smsVerfyl').val());
					if(smsVerfy==''){
						WST.msg('请输入验证码!', {icon: 5});
						return;
					}
					sendSms(goodsId,smsVerfy);
					layer.close(index2);
				},
				cancel:function(){
					isSend = false;
...
function sendSms(goodsId,smsVerfy){
		var params = {};
		params.goodsId = goodsId;
		params.smsVerfy = smsVerfy;
		$.post(WST.U('home/goods/bdResendSms'), params, function(data,textStatus){
			var json = WST.toJson(data);
			if(json.status=='1'){
				layer.alert('验证码重发成功,请凭借验证码和注册手机号到线下服务中心提货.',function(index){
					layer.close(index);
				});
			}else{
				WST.msg('操作失败!'+json.msg, {icon: 5});
			}
		});
}