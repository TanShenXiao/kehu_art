...
function login(){
    ...
    $.post(WST.U('mobile/users/checkLoginWx'), param, function(data){
...
function login2(){
    var loginName = $('#regName2').val();
    var mobileCode = $('#phoneCode2').val();
    if(loginName==''){
        WST.msg('请输入手机号','info');
        return false;
    }
    ...
    $('#loginButton').addClass("active").attr('disabled', 'disabled');
    $.post(WST.U('mobile/users/checkLoginByPhoneWx'), param, function(data){
        var json = WST.toJson(data);
        if( json.status == 1 ){
            WST.msg(json.msg,'success');
            var url = json.url;
...
function register(){
    ...
    $.post(WST.U('mobile/users/registerWx'), param, function(data){
        var json = WST.toJson(data);
        if( json.status == 1 ){
            WST.msg(json.msg,'success');
            var url = json.url;
...
var time = 0;
var isSend = false;
//type=1注册账号获取验证码,type=2手机登录获取验证码
function obtainCode(type){
    ...
    if(type==2){
        ...
        userPhone = $('#regName2').val();
        smsVerfy = $('#smsVerfy2').val();
    }
    if(userPhone ==''){
    ...
    $.post(WST.U(url), param, function(data){
        ...
        if( json.status == 1 ){
            WST.msg(json.msg,'success');
            time = 120;
            $('#obtain'+type).attr('disabled', 'disabled').html('120秒获取');
            var task = setInterval(function(){
                time--;
                $('#obtain'+type).html(''+time+"秒获取");
                if(time==0){
                    isSend = false;
                    clearInterval(task);
                    $('#obtain'+type).removeAttr('disabled').html("重新发送");
                }
            },1000);
        }else{
            WST.msg(json.msg,'warn');
