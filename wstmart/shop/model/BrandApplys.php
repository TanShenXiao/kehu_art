...
class BrandApplys extends Base{
    ...
    public function getById($id){
        ...
        $result = $this->where(['shopId'=>$shopId,'applyId'=>$id])->find();
        //获取关联的分类
    ...
    public function add(){
        ...
        try{
            ...
            if($isNew){
                ...
                if(!$validate->scene('join_add')->check($data))return WSTReturn($validate->getError());
            }
    ...
    public function edit(){
        ...
        try{
            ...
            if($isNew){
                ...
                if(!$validate->scene('join_edit')->check($data))return WSTReturn($validate->getError());
            }
            $result = $this->allowField(['brandName','brandImg','brandDesc','accreditImg','catIds'])->save($data,['shopId'=>$shopId,'applyId'=>$applyId]);
            if(false !== $result){
                WSTClearAllCache();
                Db::commit();
    ...
    public function del(){
        ...
        $data = [];
        $data['dataFlag'] = -1;
        $apply = $this->where(['shopId'=>$shopId,'applyId'=>$id])->find();
        if(empty($apply))return WSTReturn('删除失败');
        $joinIds = Db::name('brand_applys')->where(['brandId'=>$apply['brandId']])->column('applyId');
        Db::startTrans();
        try{
            ...
            if($apply['isNew']){
                ...
                if($apply['applyStatus']==1){
                    ...
                    Db::name('brand_applys')->where(['brandId'=>$apply['brandId']])->update($data);
                    // 删除推荐品牌
