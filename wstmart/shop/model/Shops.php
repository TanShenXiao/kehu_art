...
class Shops extends CShops{
    ...
    */
    ...
    public function getShopSummary($shopId){
        ...
        //待回复咨询
        $stat['consult'] = Db::name('goods_consult')->where(['shopId'=>$shopId,'dataFlag'=>1,'goodsStatus'=>1,'isSale'=>1,'isNew'=>1])->cache('newGoodsCnt'.$shopId,600)->count();
    ...
    public function editBankInfo(){
        $shopId = (int)session('WST_USER.shopId');
        $validate = new VShop;
        if (!$validate->scene('editBank')->check(input('post.'))) {
            return WSTReturn($validate->getError());
        }else{
            $result = $this->allowField(['bankId','bankAreaId','bankNo','bankUserName','bankAreaIdPath'])->save(input('post.'),['shopId'=>$shopId]);
        }
        if(false !== $result){
            return WSTReturn('操作成功!',1);
        }else{
