...
function initGrid(p){
    ...
    var cols = [
            ...
            {title:'操作', name:'' ,width:80, align:'center', renderer: function(val,item,rowIndex){
            	if(item["shopId"]>0){
            		var h = "";
		        	h += "<a class='btn btn-blue' href='javascript:toEdit("+ item['attrId']+")'><i class='fa fa-pencil'></i>修改</a> ";
		        	h += "<a class='btn btn-red' href='javascript:toDel(" + item['attrId'] + ")'><i class='fa fa-trash-o'></i>删除</a> "; 
		            return h;
            	}
            }}
            ];
