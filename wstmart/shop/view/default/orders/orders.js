...
function editOrderMoney(id){
    ...
    function deliver(id,deliverType,deliverSmsCode){
	if(deliverType==1){
        WST.confirm({content:"请确认用户提货码是否正确！<br>提货码【"+deliverSmsCode+"】",btn: ['正确，确定发货', '错误，暂不发货'], yes:function(tips){
...
//发送提货码
function sendDeliverSms(deliverId){
	var params = {};
	params.deliverId = deliverId;
	var box = WST.confirm({content:"确定重发提货码吗?",yes:function(){
		$.post(WST.U('shop/orders/sendDeliverSms'),params,function(data,textStatus){
			var json = WST.toJson(data);
			if(json.status=='1'){
				WST.msg('提货码发送成功', {icon: 6});
			}else{
				WST.msg(json.msg,{icon:2});
			}
		});
