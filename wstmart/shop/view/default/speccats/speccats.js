...
function initGrid(p){
    ...
    var cols = [
            ...
            {title:'来源', name:'attrVal', width: 20,renderer: function(val,item,rowIndex){
            	if(item["shopId"]>0){
            		return "商家规格";
            	}else{
            ...
            {title:'操作', name:'op' ,width:50, align:'center', renderer: function(val,item,rowIndex){
            	if(item.shopId>0){
            		var h = "";
			        if(item.specId>0){
			        	h += "<a class='btn btn-blue' href='javascript:toEdit("+ item['catId']+"," + item['id'] + ")'><i class='fa fa-pencil'></i>修改</a> ";
			        	h += "<a class='btn btn-red' href='javascript:toDel(" + item['id'] + ")'><i class='fa fa-trash-o'></i>删除</a> "; 
			            return h;
			        }else{
			        	h += "<a class='btn btn-blue' href='javascript:toEditCat(" + item['id'] + ")' ><i class='fa fa-pencil'></i>修改</a> ";
			        	h += "<a class='btn btn-red' href='javascript:toDelCat(" + item['id'] + ")'><i class='fa fa-trash-o'></i>删除</a> "; 
			        }
			        return h;
            	}
