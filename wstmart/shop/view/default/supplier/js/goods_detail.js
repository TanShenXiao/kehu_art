...
function checkGoodsStock(){
	...
	$('#j-supplier-price').html(goodsPrice);
	if(stock<=0){
...
function queryByPage(p){
  ...
  $.post(WST.U('shop/suppliergoodsappraises/getById'),params,function(data,textStatus){
...
function addCart(type,iptId){
	...
	$.post(WST.U('shop/suppliercarts/addCart'),{goodsId:goodsInfo.id,goodsSpecId:goodsSpecId,buyNum:buyNum,type:type,rnd:Math.random()},function(data,textStatus){
	     var json = WST.toJson(data);
	     if(json.status==1){
...
function consultCommit(){
	...
	$.post(WST.U('shop/suppliergoodsconsult/add'),params,function(responData){
		layer.close(load); 
...
function queryConsult(p){
  ...
  $.post(WST.U('shop/suppliergoodsconsult/listquery'),params,function(data,textStatus){
      var json = WST.toJson(data);
      if(json.status==1 && json.data.data){
...
function toCopy(){
	...
	$.post(WST.U('shop/Suppliergoods/checkHasCopy'),{goodsId:goodsId},function(data,textStatus){
		var json = WST.toJson(data);
		if(json.status==1){
			if(json.data.hasCopy){
				layer.open({
				    type: 1,
				    title: '系统提示',
				    shadeClose: true,
				    ...
				    shadeClose:false,
				    area: ['600px', 280 +'px'],
				    content: layui.$('#hascopy'),
				    btn: ['去查看','继续浏览'],
				    yes: function(index, layero){
				    	layer.close(index);
				    	location.href = WST.U('shop/goods/store');
				    },
				...
				WST.confirm({content:"确定将该商品复制到您的店铺吗？?",yes:function(){
			       	loading = WST.msg('正在复制商品数据，请稍后...', {icon: 16,time:60000});
			       	toCopyGoods();
			    }});
			}
...
function toCopyGoods() {
	
	var goodsId = $("#goodsId").val();
	$.post(WST.U('shop/Suppliergoods/toAdd'),{supplierGoodsId:goodsId},function(data,textStatus){
      	var json = WST.toJson(data);
      	layer.close(loading);
      	if(json.status==1){
          	layer.open({
