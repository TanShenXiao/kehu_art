...
use wstmart\weapp\model\Users as M;
use wstmart\common\model\Users as MUsers;
use wstmart\weapp\model\UserScores as MUS;
...
class Users extends Base{
    ...
    protected $beforeActionList = [
          'checkAuth' =>  ['except'=>'getlogintype,checklogin,login,loginbyphone,register,getphonecode,getphonecode2,getverify,toregister,findpass,findpassinfo,getfindphone,verifybacklogin,resetfindpass,handlelogin,checkuserphone,getfindemail,verifyemailcode']// 访问这些except下的方法不需要执行前置操作
    ];
    /*
     * 获取登录方式
     */
    public function getLoginType(){
        $loginType = WSTConf('CONF.mobileLoginType');
        $data = [];
        if($loginType!=''){
            $data = explode(',',$loginType);
        }
        return jsonReturn('ok',1,$data);
    }
    // 验证是否登录,
	...
	public function login(){
        if(strpos(WSTConf('CONF.mobileLoginType'),'1')===false)return jsonReturn("非法登录！",-1);
		$m = new M();
		return $m->login();
	}
    /**
    ...
    public function loginByPhone(){
        if(strpos(WSTConf('CONF.mobileLoginType'),'2')===false)return jsonReturn("非法登录！",-1);
        $m = new M();
        return $m->loginByPhone();
    }
    /**
     * 开启一键登录，绑定用户信息
     */
    ...
    public function getphonecode2(){
        ...
        $musers = new MUsers();
        $rs = $musers->checkUserPhone($userPhone);
        if($rs["status"]==1){
            return jsonReturn('手机号还没注册!');
        }
        $phoneVerify = rand(100000,999999);
	...
	*/
	...
	public function findPass(){
		...
		$info['userEmail'] = WSTStrReplace($info['userEmail'],'*',2,'@');
    ...
    public function getfindEmail(){
        ...
        if( $tpl['tplContent']!='' && $tpl['status']=='1'){
            ...
            $sendRs = WSTSendMail($userEmail,'密码重置',str_replace($find,$replace,$tpl['content']));
        }
        if($sendRs['status']==1){
            $USER = [];
            $USER['userEmail'] = $userEmail;
            $USER['emailVerify'] = $code;
            session('Verify_Email_Info',$USER);
            session("findPass.key", $code);
            // 发起重置密码的时间;
            session('Verify_Email_Time',time());
    ...
    public function verifyEmailCode(){
        $emailVerify = input("post.verifyCode");
        $timeVerify = session('Verify_Email_Time');
        if(!session('Verify_Email_Info.emailVerify') || time()>floatval($timeVerify)+10*60){
            return jsonReturn("校验码已失效，请重新发送！");
        }
        if($emailVerify==session('Verify_Email_Info.emailVerify')){
            return jsonReturn("验证成功",1);
        }
        return jsonReturn("校验码不一致，请重新输入！");
    }
	...
	public function verifybackLogin(){
		$phoneVerify = input("post.verifyCode");
