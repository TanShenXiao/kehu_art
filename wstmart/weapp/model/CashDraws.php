      ...
      */
      ...
      public function drawMoney(){
      	...
      	if($decrypt_data['status']==1){
      		...
      		return jsonReturn('提现申请失败');
      	}
      	$limitMoney = (float)WSTConf('CONF.drawCashUserLimit');
      	if($money<$limitMoney)return jsonReturn('提取金额必须大于或等于￥'.$limitMoney.'方可提现');
      	if($payPwd=='')return jsonReturn('支付密码不能为空');
      	//加载提现账号信息
      	...
      	if(empty($acc))return jsonReturn('提现账号不存在');
      	...
      	if($payPwd!=$user->payPwd)return jsonReturn('支付密码错误');
      	if($money>($userMoney-$rechargeMoney))return jsonReturn('提取金额不能大于用户可提现金额');
      	//减去要提取的金额
      	...
      	try{
      		...
      		if(false !==$result){
      			...
      			Db::commit();
      			return jsonReturn('提现申请成功',1);
      		}
      	}catch (\Exception $e) {
      		Db::rollback();
      		return jsonReturn('提现申请失败',-1);
      	}
      }
}
