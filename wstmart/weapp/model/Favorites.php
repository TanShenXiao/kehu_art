...
class Favorites extends Base{
	...
	public function del(){
		...
		$userId = $this->getUserId();
		$ids = explode(',',$id);
		if(empty($ids))return jsonReturn("取消失败", -1);
		$rs = $this->where([['favoriteId','in',$ids],['favoriteType','=',$type],['userId','=',$userId]])->delete();
		if(false !== $rs){
			return jsonReturn("取消成功", 1);
		}else{
			return jsonReturn('',-1,$this->getError());
		}
	}
	...
	public function add(){
		...
		if(!$isFind)return jsonReturn("关注失败，无效的关注对象", -1);
		$data = [];
		$data['userId'] = $userId;
		...
		if($rc>0)return jsonReturn("关注成功", 1);
		$data['createTime'] = date('Y-m-d H:i:s');
		$rs = $this->save($data);
		if(false !== $rs){
			// 新增关注之后,返回favoriteId,用于取消时
			return jsonReturn("关注成功", 1,['fId'=>$this->favoriteId]);
		}else{
			return jsonReturn('',-1,$this->getError());
		}
	}
