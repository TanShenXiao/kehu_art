...
class Goods extends CGoods{
	...
	*/
	...
	public function pageQuery($goodsCatIds = []){
		...
		->field('g.goodsId,goodsName,saleNum,shopPrice,goodsImg,isFreeShipping,isSelf,gs.totalScore,gs.totalUsers')
	...
	public function getBySale($goodsId){
		...
		$rs = Db::name('goods')->field('goodsDesc,costPrice',true)->where(['goodsId'=>$goodsId,'dataFlag'=>1])->find();
		if(!empty($rs)){
			$rs['read'] = false;
			//判断是否可以公开查看
			...
			$rs['shop']['freight'] = sprintf("%.2f", $rs['shop']['freight']);
			$cat = [];
	...
	public function getGoodsDetail($goodsId=0){
		// 未传递goodsId返回空数组
		if($goodsId <= 0)return [];
		$rs = Db::name('goods')->field('goodsDesc')->where(['goodsId'=>$goodsId,'dataFlag'=>1])->find();
