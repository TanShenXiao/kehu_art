...
class Shops extends CShops{
    ...
    public function pageQuery(){
    	...
    	foreach ($page['data'] as $key =>$v){
    		...
    		//总评分
            $page['data'][$key]['totalScore'] = WSTScore($v["totalScore"]/3, $v["totalUsers"]);
        ...
        $maxScore = (count($totalScores)>0 && max($totalScores)<=5)?((max($totalScores)/5)*100):100;
        $minScore = (count($totalScores)>0 && min($totalScores)>=0)?(((min($totalScores)-0.01)/5)*100):0;
        $scores = [];
        //区间跨度
    ...
    */
    public function getFloorData(){
        $limit = (int)input('currPage');
        if($limit<=0)$limit=1;
        $cacheData = cache('APP_SHOP_FLOOR'.$limit);
        if($cacheData)return $cacheData;
        //if($cacheData)return $cacheData;
        $rs = Db::name('shop_cats')->where(['dataFlag'=>1,'isShow'=>1,'parentId'=>0,'shopId'=>1])->field('catId,catName')->order('catSort asc,catId asc')->limit($limit-1,1)->select();
        if($rs){
            $rs= $rs[0];
            $goods = Db::name('goods')->where('shopCatId1','=',$rs['catId'])->where(['dataFlag'=>1,'goodsStatus'=>1,'isSale'=>1])->field('goodsId,goodsName,goodsImg,shopPrice,marketPrice,saleNum')->order('isHot desc')->limit(4)->select();
            // 图片转换
            foreach($goods as $k1=>$v1){
                $goods[$k1]['goodsImg'] = WSTImg($v1['goodsImg'],2);
            }
            $rs['goods'] = $goods;
            $rs['CurrentPage'] = $limit;
        }
        // 分类下没有商品
        //$rs['TotalPage'] = 4;// 最多取4个楼层
