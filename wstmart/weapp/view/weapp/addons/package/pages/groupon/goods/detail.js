...
Page({
  ...
  getData: function (id) {
    ...
    http.Post("addon/groupon-goods-weDetail", { tokenId: tokenId, id: id }, function (res) {
      wx.hideLoading();
      if (res.status >= 0) {
        that.title(res.goodsName);
        var nowTime = new Date(Date.parse(res.nowTime.replace(/-/g, "/")));
        var startTime = new Date(Date.parse(res.startTime.replace(/-/g, "/")));
        var endTime = new Date(Date.parse(res.endTime.replace(/-/g, "/")));
        if (res.status == 0) {
  ...
  join: function (e) {
    var that = this;
    var grouponId = that.data.grouponId;
    var sessionId = app.globalData.confInfo.sessionId;
    var tokenId = app.globalData.tokenId;
    var num = that.data.num;
    http.Post("addon/groupon-carts-weAddCart", { tokenId: tokenId, id: grouponId, buyNum: num, sessionId: sessionId, rnd: Math.random() }, function (res) {
      if (res.status == 1) {
        wx.navigateTo({
          url: '/addons/package/pages/groupon/settlement/settlement'
        });
        that.util('close');
      } else {
        app.prompt(res.msg);
        that.getData(grouponId);
      }
    });
  },
  onRoll: function (e) {
  ...
  onShareAppMessage: function (res) {
    ...
    return {
      ...
      path: '/addons/package/pages/groupon/goods/detail?id=' + that.data.grouponId
    }
  },
  //评论
