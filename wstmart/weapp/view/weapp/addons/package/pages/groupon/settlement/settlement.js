...
Page({
  ...
  */
  ...
  getData: function () {
    ...
    http.Post("addon/groupon-carts-weSettlement", { sessionId: sessionId, tokenId: tokenId, addressId: addressId }, function (res) {
      if (res.status == 1) {
  ...
  getTotal: function () {
    ...
    var useScore = data.userOrderScore ? data.userOrderScore:0;
    var tokenId = app.globalData.tokenId;
    var goodsType = that.data.goodsType;
    if (goodsType == 1) deliverType = 1;
    http.Post("addon/groupon-carts-weGetCartMoney", { tokenId: tokenId, areaId2: areaId2, deliverType: deliverType, isUseScore: isUseScore, useScore: useScore }, function (res) {
      if (res.status == 1) {
  ...
  togoods: function (e) {
    ...
    wx.navigateTo({
      url: '/addons/package/pages/groupon/goods/detail?id=' + tuanId
    })
  },
  /*跳转到地址管理 */
  address: function () {
  ...
  submit: function () {
    ...
    http.Post("addon/groupon-carts-weSubmit", datas, function (res) {
      wx.hideLoading();
      if (res.status == 1) {
