...
Page({
  ...
  getData: function () {
    ...
    http.Post("addon/integral-goods-welists", { tokenId: tokenId }, function (res) {
      if (res.status == 1) {
        ...
        that.setData({
          userMoney: userInfo.userMoney ? userInfo.userMoney :0.00,
          userScore: userInfo.userScore ? userInfo.userScore : 0,
          goodsCats: goodsCats,
  ...
  goodsList: function () {
    ...
    params.page = page;
    that.setData({ load: 0 });
    http.Post("addon/integral-goods-wePageQuery", params, function (res) {
      if (res.status == 1) {
