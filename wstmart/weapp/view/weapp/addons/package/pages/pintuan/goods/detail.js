...
Page({
  ...
  getData: function (id) {
    ...
    http.Post("addon/pintuan-goods-weDetail", { tokenId: tokenId, id: id }, function (res) {
      wx.hideLoading();
      if (res.status >= 0) {
        ...
        that.setData({
          ...
          isSpec: res.isSpec,
          goodsSpecId: res.goodsSpecId ? res.goodsSpecId :0,
          gallery: gallery,
          goodsType: res.goodsType,
          favGood: res.favGood,
          goodsId: res.goodsId,
          descImage: res.goodsImg,
          goodsStock: res.goodsStock,
          tuanStock: (res.goodsNum > 0) ? (res.goodsNum):0
        });
        var pulist = that.data.pulist;
        var nowTime = that.data.nowTime;
        var timers = [];
        for (var i = 0; i < pulist.length; i++) {
          var startTime = new Date(Date.parse(pulist[i].createTime.replace(/-/g, "/")));
          var endTime = new Date(Date.parse(pulist[i].endTime.replace(/-/g, "/")));
  ...
  powerDrawer: function (e) {
    ...
    var maxStock = (cartsType == 1) ? that.data.goodsStock : that.data.tuanStock;
    var goodsPrice = (cartsType == 1) ? that.data.goods.shopPrice : that.data.goods.tuanPrice;
    that.setData({
      cartsType: cartsType,
      maxStock: maxStock,
  ...
  join: function (e) {
    ...
    if (cartsType==1){
      http.Post("weapp/carts/addCart", { tokenId: tokenId, goodsId: goodsId, goodsSpecId: goodsSpecId, buyNum: num, types: types, sessionId: sessionId, rnd: Math.random() }, function (res) {
        if (res.status == 1) {
          if (types == 1) {
            if (goodsType == 1) {
              wx.navigateTo({
                url: '/pages/settlement-quick/settlement-quick'
              })
            } else {
              wx.navigateTo({
                url: '/pages/carts/carts'
      ...
      http.Post("addon/pintuan-carts-weAddCart", { tokenId: tokenId, id: tuanId, buyNum: num, tuanType: 1, sessionId: sessionId, rnd: Math.random() }, function (res) {
        if (res.status == 1) {
          wx.navigateTo({
  ...
  onShareAppMessage: function (res) {
    ...
    return {
      ...
      path: '/addons/package/pages/pintuan/goods/detail?id=' + that.data.tuanId
    }
  },
  //评论
  ...
  toPintuan:function(e){
    ...
    wx.navigateTo({
      url: '/addons/package/pages/pintuan/users/tuan_detail?tuanNo=' + tuanNo + "&vtype=1"
    })
  },
  getNowTime: function () {
    var that = this;
  ...
  countDown: function (nowTime, endTime, goodIndex) {
    ...
    var opts = {
      ...
      callback: function (data) {
        if (data.last > 0) {
          that.data.pulist[goodIndex]['hour'] = data.hour;
          that.data.pulist[goodIndex]['mini'] = data.mini;
          that.data.pulist[goodIndex]['sec'] = data.sec;
          that.data.pulist[goodIndex]['msec'] = data.msec;
          that.setData({
            pulist: that.data.pulist,
          });
        } else {
          that.data.pulist[goodIndex]['status'] = -1;
          that.setData({
            pulist: that.data.pulist,
          });
        }
