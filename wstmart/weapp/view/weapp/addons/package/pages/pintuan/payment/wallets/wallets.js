...
Page({
  ...
  */
  ...
  getData:function(){
    ...
    http.Post("addon/pintuan-walletswe-getWalletsUrl", postData, function (res) {
      if (res.status == 1) {
         ...
         http.Get('addon/pintuan-walletswe-wallets',{
          ...
          tokenId:tokenId,
            ...
            if (payInfo.status != 1){
              return that.goBack('获取拼团支付信息失败');
            }
            that.setData({
                data:payInfo.data,
                pwdType: payInfo.data.payPwd,
                needPay:payInfo.data.needPay,
                userMoney:payInfo.userMoney,
                pkey:res.pkey
            });
         });
      }else{
  ...
  payment:function(e){
    ...
    http.Post("addon/pintuan-walletswe-payByWallet", { tokenId: tokenId, payPwd: payPwd, pkey: pkey}, function (res) {
      wx.hideLoading();
      if (res.status == 1) {
        wx.navigateTo({
