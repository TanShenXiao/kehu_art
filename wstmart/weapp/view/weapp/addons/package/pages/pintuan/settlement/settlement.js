...
Page({
  ...
  */
  ...
  getData:function(){
    ...
    http.Post("addon/pintuan-carts-weSettlement", { sessionId: sessionId, tokenId: tokenId, addressId: addressId}, function (res) {
      if (res.status == 1) {
  ...
  getTotal:function(){
    ...
    http.Post("addon/pintuan-carts-weGetCartMoney", { tokenId: tokenId, areaId2: areaId2, deliverType: deliverType, isUseScore: isUseScore, useScore: data.userOrderScore}, function (res) {
      if (res.status == 1){
        data["userOrderScore"] = res.data.maxScore;
        that.setData({
          price: res.data,
          data: data
        })
  ...
  togoods: function (e) {
    ...
    wx.navigateTo({
      url: '/addons/package/pages/pintuan/goods/detail?id=' + tuanId
    })
  },
  /*跳转到地址管理 */
  address:function (){
  ...
  submit:function(){
    ...
    http.Post("addon/pintuan-carts-weSubmit", datas, function (res) {
      wx.hideLoading();
      if (res.status == 1) {
        if (payType == 1 && price.realTotalMoney>0){
          wx.showToast({
            ...
            complete: function (err) {
              if (payCode == 'weixinpays' || payCode == ''){
                wx.redirectTo({
                  url: '/addons/package/pages/pintuan/payment/weixinpays/weixinpays?pkey=' + res.pkey
                })
              } else if (payCode == 'wallets'){
                wx.redirectTo({
                  url: '/addons/package/pages/pintuan/payment/wallets/wallets?pkey=' + res.pkey
                })
              }
            }
          })
          ...
          wx.showToast({
            ...
            complete: function (err) {
              wx.redirectTo({
                url: '/addons/package/pages/pintuan/users/pulist',
              })
            }
          })
        }
