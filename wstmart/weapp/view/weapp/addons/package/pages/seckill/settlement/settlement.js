...
Page({
  ...
  */
  ...
  getData: function (id) {
    ...
    http.Post("addon/seckill-carts-weSettlement", { sessionId: sessionId, tokenId: tokenId, addressId: addressId}, function (res) {
      wx.hideLoading();
      if (res.status == 1) {
  ...
  getTotal: function () {
    ...
    var tokenId = app.globalData.tokenId;
    http.Post("addon/seckill-carts-weGetCartMoney", { tokenId: tokenId, deliverType: deliverType, isUseScore: isUseScore, useScore: data.userOrderScore, areaId2: areaId2 }, function (res) {
      if (res.status == 1) {
        that.setData({
          prices: res.data
        })
  ...
  submit: function () {
    ...
    http.Post("addon/seckill-carts-weSubmit", datas, function (res) {
      wx.hideLoading();
      if (res.status == 1) {
          ...
          wx.showToast({
            ...
            complete: function (err) {
              if (payCode == 'weixinpays' || payCode == '') {
                wx.redirectTo({
                  url: '/pages/users/payment/weixinpays/weixinpays?orderNo=' + res.data
                })
              } else if (payCode == 'wallets') {
                wx.redirectTo({
                  url: '/pages/users/payment/wallets/wallets?orderNo=' + res.data
                })
              }
            }
          })
