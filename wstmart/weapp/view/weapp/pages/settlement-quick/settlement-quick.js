...
Page({
  ...
  */
  ...
  getData:function(){
    ...
    http.Post("weapp/carts/quickSettlement", { tokenId: tokenId, sessionId: sessionId}, function (res) {
      if (res.status == 1) {
        ...
        that.setData({
          ...
          ca:res.data.carts[res.data.shopId]
        })
        that.getTotal();
      }
    });
  ...
  getTotal:function(){
    ...
    http.Post("weapp/carts/getQuickCartMoney", { tokenId: tokenId, isUseScore: isUseScore, useScore: data.userOrderScore,sessionId: sessionId, couponIds: couponIds}, function (res) {
      if (res.status == 1){
        that.setData({
          price: res.data,
        })
  ...
  submit:function(){
    ...
    if (shopRemarks[data.shopId]) {
      datas['remark_' + data.shopId] = shopRemarks[data.shopId];
    } else {
      datas['remark_' + data.shopId] = '';
    }
    if (addons.Coupon == 1) {
      datas['couponId_'+data.shopId] = coupons[data.shopId]['couponId'];
    }
    http.Post("weapp/orders/quickSubmit", datas, function (res) {
      wx.hideLoading();
      if (res.status == 1) {
  ...
  onCoupon: function (e) {
    ...
    that.setData({
      couponsInfo: coupons[shopId]?coupons[shopId]:[],
      couponsshopId: shopId?shopId:0,
      tag:false
    })
  },
  ...
  ifCoupon:function(e){
    ...
    if (couponId==0){
      ...
      coupons[shopId]['derate'] = 0.00;
      coupons[shopId]['derateTxt'] = false;
    }else{
      for (var co in coupons[shopId]['list']){
        if (coupons[shopId]['list'][co]['couponId'] == couponId){
          ...
          coupons[shopId]['derate'] = coupons[shopId]['list'][co]['couponValue']+ ".00";
          coupons[shopId]['derateTxt'] = true;
        }
      }
    }
    that.setData({
