...
Page({
  ...
  */
  ...
  getData:function(){
    try{
      WSTAddons['reward'] = require('../../addons/closure/reward/reward.js');
    }catch(e){}
    app.emptyHook('settlementGoodsPromotion');
    var that = this;
    ...
    http.Post("weapp/carts/settlement", { tokenId: tokenId, addressId: addressId}, function (res) {
      if (res.status == 1) {
        if (addons.Reward == 1) {
            if(typeof(WSTAddons['reward']['initSettlement'])=='function')WSTAddons['reward']['initSettlement'](res,{ tokenId: tokenId});
        }
