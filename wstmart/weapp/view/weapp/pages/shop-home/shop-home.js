    ...
    http.Post('weapp/shops/home', {
      ...
      if (res.status == 1 && res.data) {
        ...
        if (shopArray.shopBanner != '' && shopArray.shopBanner != 'null'){
          var shopBanner = that.data.resourceDomain+shopArray.shopBanner
        }else{
          var shopBanner = '/image/typeimage/default_shopbanner.jpg'
        }
        var isLocation = (res.data.shop.longitude && res.data.shop.latitude)?true:false;
        that.setData({
          shopArray: shopArray,
          hasattension: hasattension,
          sortInfo: sortInfo,
          hots:shopArray.shopHotWords,
          isFavor: isFavor,
  ...
  powerDrawer(e) {
    if (e.currentTarget.dataset.page !=2) {
      ...
      this.setData({
        ct2:0,
        ct1:0,
        inputText: e.currentTarget.dataset.key,
        page:1,
        assifycommodity:[]
      });
      this.search();
