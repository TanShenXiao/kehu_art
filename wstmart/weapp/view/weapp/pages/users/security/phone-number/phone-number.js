...
Page({
  ...
  getPhoneNumber: function (e) {
      ...
      var iv = e.detail.iv;
      var sessionKey = app.globalData.sessionKey ? app.globalData.sessionKey:'';
      var unionKey = app.globalData.unionKey;
      var tokenId = app.globalData.tokenId;
      if (e.detail.errMsg == 'getPhoneNumber:ok') {
        http.Post("weapp/request/bizPhone", { tokenId:tokenId,encryptedData: encryptedData, iv: iv, sessionKey: sessionKey, unionKey: unionKey }, function (res) {
          switch (res.status) {
            case 1:
              app.prompt(res.msg);
              wx.navigateTo({
    ...
    if (e.detail.errMsg == 'getUserInfo:ok') {
      app.getUserInfo(function (res) {
        if (app.globalData.confInfo.weappOneClickLogin == 1) {
          ...
          var tokenId = app.globalData.tokenId;
          http.Post("weapp/users/bindUserInfo", { tokenId: tokenId, sessionKey: sessionKey, unionKey: unionKey, avatarUrl: avatarUrl, nickName: nickName, gender: gender }, function (res) {
            if (res.status == 1) {
              // 绑定用户信息成功
              app.prompt('授权成功');
              that.setData({
