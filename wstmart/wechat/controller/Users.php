...
class Users extends Base{
    ...
    protected $beforeActionList = [
        'checkAuth' =>  ['except'=>'index,checklogin,checkloginbyphone,login,register,getverify,getphoneverifycode,getphoneverifycode2,checkuserphone,forgetpass,forgetpasst,forgetpasss,findpass,resetpass,getfindphone,checkfindphone,getfindemail']// 访问这些except下的方法不需要执行前置操作
    ];
    /**
     * 会员登录页
     */
    ...
    protected function expire($msg=''){
    	...
    	$html = '<h1>'.$message.'</h1><script>setTimeout(function(){location.href="'.url('wechat/users/index','','',true).'";},1000)</script>';
    ...
    public function checkfindPhone($phoneVerify){
    	...
    	if (session('findPhone.phoneVerify') == $phoneVerify ) {
    		...
    		if(!empty($fuserId)){
    			...
    			session('REST_success','1');
    			$rs['status'] = 1;
    			$rs['url'] = url('wechat/users/resetPass');
    			return $rs;
    		}
