...
//登录
WST.inLogin = function(redirectUrl){
	var urla = redirectUrl || window.location.href;
...
WST.intoShops = function(id){
	location.href = WST.U('wechat/shops/index','shopId='+id);
};
//首页
...
WST.search = function(type){
	...
	if(type==1){
		...
		location.href = WST.U('wechat/goods/search','keyword='+data);//商品
	}else if(type==2){
		var shopId = $('#shopId').val();
		location.href = WST.U('wechat/shops/goods','goodsName='+data+'&shopId='+shopId);//店铺商品
	}else if(type==3){
...
WST.upload = function(opts){
  var _opts = {};
  _opts = $.extend(_opts,{duplicate:true,auto: true,swf: WST.conf.STATIC +'/plugins/webuploader/Uploader.swf',server:WST.U('wechat/orders/uploadPic')},opts);
...
WST.recordThumb = function(goodsId,userId,thumbsNum,shopOwnerId){
	if(userId > 0){
		if(userId==shopOwnerId){
			WST.msg("不能给自己的商品点赞",'info');
			return;
		}
		$.post(WST.U('wechat/goods/recordThumb'),{goodsId:goodsId,userId:userId},function(data){
			if('success'==data) {
				$('#dz').addClass('dz-red');
				$('#dz').removeClass('dz');
