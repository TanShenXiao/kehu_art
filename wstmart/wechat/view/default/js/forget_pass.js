...
function forgetPwd(){
    ...
    $.post(WST.U('wechat/users/findPass'),params,function(data,textStatus){
      var json = WST.toJson(data);
      if(json.status=='1'){
	        	if(step==1){
		        	location.href=WST.U('wechat/users/forgetPasst','',true);
	        	}else if(step==2){
	        		if(modes==1){
...
function resetByEmail(){
	...
	if(code == ''){
		...
		$.post(WST.U('wechat/users/forgetPasss'),{secretCode:code},function(data){
			var json = WST.toJson(data);
			if(json.status==-1){
				WST.msg('校验码错误','info');
				WST.getVerify("#verifyImg1")
				...
				location.href=WST.U('wechat/users/resetPass','',true);
			}
		})
	}	
}
...
function resetPwd(){
	...
	$.post(WST.U('wechat/users/findPass'),params,function(data){
		var json = WST.toJson(data);
		if(json.status==1){
			WST.msg('重置成功');
			setTimeout(function(){
				location.href=WST.U('wechat/users/login');
			},1000)
		}else{
			WST.msg(json.msg,'info');
			return false;
...
function phoneVerify2(){
	...
	$.post(WST.U('wechat/users/getfindPhone'),{smsVerfy:smsVerfy},function(data,textStatus){
		var json = WST.toJson(data);
		if(isSend )return;
		isSend = true;
		if(json.status!=1){
