...
$(function(){
    ...
    WST.imgAdapt('j-imgIndex1');
	imgAdaptHalf('half-img');
    // 商品分类
	...
	var img_h = $('.wst-in-goods .img').css('height');
	var new_h = parseInt(img_h)+48;
	$('.wst-in-adsb').css('height',new_h);
	$('#showMore').click(function(){
		$('#shop-brief').css({'-webkit-line-clamp': '999'});
		$('#showMore').hide(),$('#exMore').show();
	});
	$('#exMore').click(function(){
		$('#shop-brief').css({'-webkit-line-clamp': '2'});
		$('#exMore').hide(),$('#showMore').show();
	});
	$('#showMore1').click(function(){
		$('#shop-brief1').css({'-webkit-line-clamp': '999'});
		$('#showMore1').hide(),$('#exMore1').show();
	});
	$('#exMore1').click(function(){
		$('#shop-brief1').css({'-webkit-line-clamp': '2'});
		$('#exMore1').hide(),$('#showMore1').show();
	});
});

//适应图片长方形
imgAdaptHalf = function(name){
	var w = $('.'+name).width();
	var h = w/2;
	$('.'+name).css({"width": w+"px","height": h+"px"});
	$('.'+name+' a').css({"width": w+"px","height": h+"px"});
	$('.'+name+' a img').css({"max-width": w+"px","max-height": h+"px"});
}
...
function shopAds(){
	...
	//中间小广告	
	if($('.wst-in-adst a').hasClass("advert3")){
...
function shopsList(){
    ...
    $.post(WST.U('wechat/shops/getShopGoods'), param, function(data){
        ...
        if(json && json.data && json.data.length>0){
            ...
            html += '<div class="wst-prompt-icon"><img src="'+ window.conf.MOBILE +'/img/nothing-goods.png"></div>';
...
function getGoodsList(ct1){
	...
	location.href=WST.U('wechat/shops/goods',{'shopId':shopId,'ct1':ct1},true)
}
