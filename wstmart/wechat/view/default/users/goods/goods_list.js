...
function getBdGoodsList(){
  $('#Load').show();
    ...
    param.deliverType = -1;
    param.payType = -1;
    $.post(WST.U('wechat/goods/myGoodsByPage'), param, function(data){
...
function backSale(id){
	...
	$.post(WST.U('wechat/goods/backSale'), params, function(data,textStatus){
		var json = WST.toJson(data);
		if(json.status=='1'){
...
function delivery(id){
	var params = {};
	params.ids = {goodsId:id};
	$.post(WST.U('wechat/goods/bdDelivery'), params, function(data,textStatus){
		var json = WST.toJson(data);
		if(json.status=='1'){
