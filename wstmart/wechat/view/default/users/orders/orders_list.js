...
function addCart(goodsId, goodsSpecId, goodsType){
	$.post(WST.U('wechat/carts/addCart'),{goodsId:goodsId,goodsSpecId:goodsSpecId,buyNum:1,rnd:Math.random()},function(data,textStatus){
	     var json = WST.toJson(data);
	     if(json.status==1){
	    	 WST.msg(json.msg,'success');
	    	 if(type==1){
...
function afterSale(orderId){
  location.href = WST.U('wechat/orderservices/apply',{orderId:orderId});
}
...
function choicePay(pkey){
	location.href=WST.U('wechat/orders/succeed',{'pkey':pkey});
}
//跳转支付
function toPay(pkey,n){
	if(n=='weixinpays'){
		location.href=WST.U('wechat/weixinpays/toPay',{'pkey':pkey});
	}else if(n=='wallets'){
		location.href = WST.U('wechat/wallets/payment',{"pkey":pkey});
	}
}