  ...
  $('.ui-icon-chooseg').click(function () {
    ...
    var uploader =WST.upload({
        ...
        callback:function(f,file){
          ...
          if(json.status==1){
          var tdiv = $("<li>"+
                       "<img class='imgSrc' src='"+WST.conf.RESOURCE_PATH+"/"+json.savePath+json.thumb+"' v='"+json.savePath+json.name+"'></li>");
...
function choseReason(n){
    var _obj = $('#stBox .ui-icon-checked-s');
    var _val = _obj.attr('val');
    $('#serviceType').val(_val);
    console.log('_val', _val);
    $('#sTText').html(_obj.attr('word'));
    dataHide(n);
}
// 售后类型
function choseType(n){
    var _obj = $('#stTypeBox .ui-icon-checked-s');
    var _val = _obj.attr('val');
...
function getCanRefundMoney(){
    ...
    $.post(WST.U('wechat/orderservices/getRefundableMoney'),_glist,function(res){
...
function commitOrderService(){
    ...
    $.post(WST.U('wechat/orderservices/commit'), postData, function(res){
      var json = WST.toJson(res);
      if(json.status==1){
        history.go(-1);
      }
