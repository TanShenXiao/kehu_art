...
function pagequery(){
    $('#Load').show();
      ...
      $.post(WST.U('wechat/orderservices/pagequery'), param, function(data){
...
function viewDetail(id){
    var url = WST.U('wechat/orderservices/detail',{id:id});
    location.href = url;
}
//  隐藏对话框
...
function doConfirm(postData){
    $.post(WST.U('wechat/orderservices/userReceive'),postData,function(res){
        var json = WST.toJson(res);
        WST.msg(json.msg);
        if(json.status==1){
            // 返回列表页
            location.href = WST.U('wechat/orderservices/oslist');
        }
    })
}
